...
class User {
    ...
    function detail(array & $request, array & $response, array & $app) {
        ...
        if (isset($request['_id'])) {
            $ret = load_model('sys/UserModel')->get_by_id($request['_id']);
        } else {
            $ret = load_model('sys/UserModel')->get_by_id(CTX()->get_session("user_id"));
        }
    ...
    function do_edit(array & $request, array & $response, array & $app) {
        ...
        $ret = load_model('sys/UserModel')->update($user, $request['user_id'], $sys_data);
    ...
    function do_chgpasswd(array & $request, array & $response, array & $app) {
        ...
        if (isset($request["do"])) {
            ...
            $oldpwd = load_model('sys/UserModel')->getMd5_ToBase64($oldpwd);  //加密操作
            $ret = load_model('sys/UserModel')->getuser_pwd(CTX()->get_session("user_id"));
            if ($ret != "") {
                if ($oldpwd != $ret) {//原密码验证不通过
                    exit_json_response(load_model('sys/UserModel')->format_ret("-1", '', '原密码错误'));
                } else {
                    //更新密码
                    $newpwd = $request["new_user_pwd"];  //获取新密码
                    $newpwd = load_model('sys/UserModel')->getMd5_ToBase64($newpwd);  //加密操作
                    $result = load_model('sys/UserModel')->updatepwd(CTX()->get_session("user_id"), $newpwd);
