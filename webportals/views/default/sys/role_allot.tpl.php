...
<style>
    ...
    .baocun-box{ margin:20px 10px 20px 10px;}
    ...
    #tbl_1 td{padding:4px;border-left:1px #ccc solid;border-bottom:1px #ccc solid;}
    #tbl_1 .intd{padding:0;border:0px;}
    #tbl_1 .tdm4{width:300px;}
    #tbl_1 .tdm4 label{float:left;width:100px;}
</style>
<?php
...
<div id="purview_menu" style="margin-left: 8px">
    <table id="tbl_th">
        <tr><th>一级菜单</th><th>二级菜单</th><th>三级菜单</th><th style="width:300px;">功能按钮</th></tr>
    </table>
    <table id="tbl_1">
        <?php
        foreach ($response['menu_tree'] as $m1) {
            ...
            foreach ($m1['_child'] as $m2) {
                ...
                foreach ($m2['_child'] as $m3) {
                    ...
                    echo "<tr><td><label><input type='checkbox' level='3' m1v='{$m1['action_id']}' m2v='{$m2['action_id']}' value='{$m3['action_id']}' {$chk_tag_3}/>{$m3['action_name']}</label></td><td class='tdm4'>";
                    if (isset($m3['_child'])) {
                        foreach ($m3['_child'] as $m4) {
                            ...
                            echo "<label><input type='checkbox' level='4' m1v='{$m1['action_id']}' m2v='{$m2['action_id']}' m3v='{$m3['action_id']}' value='{$m4['action_id']}' {$chk_tag_4}/>{$m4['action_name']}</label>";
                        }
                    } else {
                        echo "<label>&nbsp;</label>";
                    }
...
<div class="addListSubmit">
    <div class="baocun-box">
        <input type="button" value="全选" class="button button-primary" onclick="checkBoxAll(jQuery('#purview_menu'))"/>
        <input type="button" value="反选" class="button button-primary" onclick="checkBoxNoAll(jQuery('#purview_menu'))"/>
    ...
    $("#tbl_1 input").click(function () {
        ...
        if (level == 3) {
            if (chk_flag) {
                ...
                $(":checkbox[m3v=" + cur_v + "]").attr("checked", true);
            } else {
                $(":checkbox[m3v=" + cur_v + "]").removeAttr("checked");
            }
        }
        if (level == 4) {
            if (chk_flag) {
