-- ----------------------------
-- Table structure for api_youzan_order 有赞订单明细
-- ----------------------------
DROP TABLE IF EXISTS `api_youzan_order`;
CREATE TABLE `api_youzan_order` (
  ...
  `buyer_messages` varchar(255) DEFAULT NULL  COMMENT '交易明细中的买家留言列表(json)',
  `order_promotion_details` varchar(255) DEFAULT NULL  COMMENT '交易明细中的优惠信息列表(json)',
  PRIMARY KEY (`id`),
  KEY `num_iid` (`num_iid`) USING BTREE,
  KEY `sku_id` (`sku_id`) USING BTREE,
  KEY `tid` (`tid`) USING BTREE
) ENGINE=InnoDB AUTO_INCREMENT=1 DEFAULT CHARSET=utf8 COMMENT='有赞订单明细表';
