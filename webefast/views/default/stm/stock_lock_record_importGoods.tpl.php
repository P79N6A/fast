    ...
    <div class="row form-actions actions-bar">
        <div class="span13 offset3 ">
            <?php if($response['lof_status'] == 1){?>
                <a class="button" style="float: left;" target="_blank" href="<?php echo get_excel_url("stock_lock_record_lof.xlsx",1) ?>">模版下载</a>
            <?php }else{ ?>
                <a class="button" style="float: left;" target="_blank" href="<?php echo get_excel_url("stock_lock_record.xlsx",1) ?>">模版下载</a>
            <?php } ?>
...
<div class="result1" style="display: block">
    <?php if($response['import_from']=='已锁定'){ ?>
        <font color="red"> 提示：若导入商品已存在，则导入无效</font>
    <?php }else{ ?>
        ...
        var uploader = new Uploader.Uploader({
            ...
            success: function (result) {
                BUI.Message.Confirm('确定要导入数据吗？',function(){
                    ...
                    var r = '?app_act=stm/stock_lock_record/do_import_goods&app_fmt=json&is_lof=<?php echo $response['lof_status'] ?>&id=' + id + '&import_from=<?php echo $response['import_from'] ?>';
                    var params = {"url": url};
                    var msg = '';
