    ...
    <div class="row form-actions actions-bar">
        <div class="span13 offset3 ">
            <?php if($response['lof_status'] == 1){?>
                <a class="button" style="float: left;" target="_blank" href="<?php echo get_excel_url("stock_adjust_record_lof.xlsx",1) ?>">模版下载</a>
            <?php }else{?>
            <a class="button" style="float: left;" target="_blank" href="<?php echo get_excel_url("stock_adjust_record.xlsx",1) ?>">模版下载</a>
            <?php } ?>
...
<script type="text/javascript">
    ...
    BUI.use('bui/uploader', function (Uploader) {
         ...
         */
        var url = '?app_act=stm/stock_adjust_record/import_goods&app_fmt=json';
        var filetype = {
        ...
        var uploader = new Uploader.Uploader({
            ...
            success: function (result) {
                if (confirm('确定要导入数据吗')) {
                    ...
                    var r = '?app_act=stm/stock_adjust_record/do_import_goods&app_fmt=json&is_lof=<?php echo $response['lof_status'] ?>&id=' + id;
                    var params = {"url": url};
                    var msg = '';
    ...
    $(document).ready(function () {
        $("#submit_import_goods").click(function () {
            ...
            $("#submit_import_goods").html('<i class="icon-plus-sign icon-white"></i>' + ' 导入中...');
            $("#submit_import_goods").attr("disabled", true);
            var r = '?app_act=stm/stock_adjust_record/do_import_goods&app_fmt=json&id=' + id;
            var params = {"url": url};
            var msg = '';
            $.post(r, params, function (data) {
                var type = data.status == 1 ? 'success' : 'error';
