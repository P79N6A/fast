...
<div class="scan_wrap">
    ...
    <div id="success_tips" style="display:none">
        <div class="tips tips-small  tips-success"> <!--  <span class="x-icon x-icon-small x-icon-success"></span>-->
            <div class="tips-content">扫描出库成功</div>
        </div>
    </div>
    ...
    <div class="scan_div">
        ...
        <input type="button" id="ys_btn" value="扫描出库"/>
        ...
        <table id="sku_tbl">
            ...
            <?php foreach($response['scan_data'] as $sub_mx) {?>
                <tr>
                    ...
                    <td class="smsl"><?php echo "<span name='sku_num_{$sub_mx['barcode']}'>{$sub_mx['scan_num']}</span>";?><?php echo "<span name='gb_sku_num_{$sub_mx['gb_code']}' style='display:none'>{$sub_mx['scan_num']}</span>";?></td>
...
<script type="text/javascript">
    ...
    function update_total_scan_sl() {
        ...
        for (var i in g_must_scan_mx) {
            total_scan_sl += parseInt(g_must_scan_mx[i]['scan_num']);
        }
    ...
    function scan_barcode(){
        ...
        var url = '<?php echo get_app_url('stm/store_shift_record/store_out_scan'); ?>';
    ...
    function scan_barcode_update(data){
        ...
        if (g_must_scan_mx[sku]){
            g_must_scan_mx[sku]['scan_num'] = parseInt(data.num);
        }
        $("span[name='sku_num_"+scan_barcode+"']").html(g_must_scan_mx[sku]['scan_num']);
        $("span[name='gb_sku_num_"+scan_barcode+"']").html(g_must_scan_mx[sku]['scan_num']);
...
//	$("#total_no_scan_sl").html(g_total_sl - g_must_scan_mx[sku]['in_num']);
    ...
    function check_scan_end(){
        var is_end = 1;
        for(var i in g_must_scan_mx){
            if (parseInt(g_must_scan_mx[i]['out_num'])>parseInt(g_must_scan_mx[i]['scan_num'])){
                is_end = 0;
                break;
            }
        }
    ...
    function ys_record(){
        if (!check_scan_end()){
            if (confirm('还未扫描完毕，商品数量存在差异，是否出库？')){
                ys_act();
            }
            return;
        }
    ...
    function clean_scan(){
        BUI.Message.Confirm('确认要清除所有扫描记录吗？',function(){
            ...
            data = {id: pid,record_code:record_code,out_type:'scan_out'};
            $.ajax({
                type: 'POST', dataType: 'json',
                url: url,
                data: data,
