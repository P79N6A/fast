        ...
        <div class="well" style="margin-left: 1em;">
            <form id="skuSearchForm" name="skuSearchForm" >
                <div style="margin: 10px;">
                    ...
                    <input type="text" class="input" placeholder="档期号" name="po_no"/>
    ...
    $(function () {
        ...
        BUI.use(['bui/grid', 'bui/data', 'bui/form', 'bui/tooltip'], function (Grid, Data, Form, Tooltip) {
            ...
            skuSelectorStore = new Data.Store({
                url: '?app_act=api/api_weipinhuijit_po/select_action&app_fmt=json',
                autoLoad: true, //自动加载数据
                autoSync: true,
                pageSize: page_size,	// 配置分页数目
                params:{pick_po_no:'<?php echo $request['pick_po_no'];?>',po_no_except:'<?php echo $request['po_no_except']?>',shop_code:'<?php echo $request['shop_code']?>'}
            });
            ...
            //渲染结果列表或刷新结果列表
            var columns = [
                {title: '档期号', dataIndex: 'po_no', width: '25%', 'sortable': false},
                {title: '开始时间', dataIndex: 'sell_st_time', width: '25%', 'sortable': false},
                {title: '结束时间', dataIndex: 'sell_et_time', width: '25%', 'sortable': false},
                {title: '店铺', dataIndex: 'shop_code_name', width: '25%', 'sortable': false},
            ];
