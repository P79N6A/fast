    ...
    <div class="row form-actions actions-bar">
        <div class="offset3 ">
            <?php if ($response['lof_status'] == 1) { ?>            
                <a class="button" target="_blank" style="float: left;" href="<?php echo get_excel_url("return_record_lof.csv",1) ?>">模版下载</a>
            <?php } else { ?>
                <a class="button" target="_blank" style="float: left;" href="<?php echo get_excel_url("return_record.csv",1) ?>">模版下载</a>
            <?php } ?> 
...
// 	            minSize: [1, '文件最小不能小于1k!'],
        ...
        var uploader = new Uploader.Uploader({
            ...
            success: function (result) {
                if (confirm('确定要导入数据吗')) {
                    var url = result.url;
                    var r = '?app_act=pur/return_record/import_goods_upload&is_lof=<?php echo $response['lof_status'] ?>&app_fmt=json&id=' + id;
                    var params = {"url": url};
                    var msg = '';
