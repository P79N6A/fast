<style type="text/css">
    ...
    .spec1-name {display:inline-block; width:auto; height:20px; overflow:hidden; vertical-align:text-bottom; font-size:12px;}
</style>
...
<div id="panel" class="">
    <form action="?app_act=prm/goods/<?php echo $response['action']; ?>" id="form1" method="post">
        <table class='table_panel'   id='p1'>
            ...
            <tr><td style="width:108px;">商品编码：<b style="color:red"> *</b></td><td><input id="goods_code" class="bui-form-field" type="text"  value="<?php echo $response['data']['goods_code']; ?>" name="goods_code" data-rules="{required : true}" aria-disabled="false" aria-pressed="false" <?php if ($response['action'] == 'do_edit') { ?> disabled="disabled" <?php } ?>></td></tr>
            <tr><td style="width:108px;">商品名称：<b style="color:red"> *</b></td><td><input id="goods_name" class="bui-form-field" type="text"  value="<?php echo $response['data']['goods_name']; ?>" name="goods_name" data-rules="{required : true}"  aria-disabled="false" aria-pressed="false"></td></tr>
            <tr><td style="width:108px;">组装商品：</td><td>
                        <select disabled="disabled" >
                            <option  value ="0" >否</option>
                        </select>
                        <input type="hidden" name="diy" id="diy" value="0">
                </td></tr>
            <tr>
            ...
            <tr>
                <td style="width:108px;">商品供应商：</td><td>
                    <select name="supplier_code" id="supplier_code">
                        <option value ="">请选择供应商</option>
                        <?php foreach ($response['supplier'] as $k => $v) { ?>
                            <option  value ="<?php echo $v['supplier_code']; ?>" <?php if ($response['data']['supplier_code'] === $v['supplier_code']) { ?> selected <?php } ?> ><?php echo $v['supplier_name']; ?></option>
                        <?php } ?>
                    </select>
                    <a href='#' id='supplier_select'><img id="supplier_select_img" src="assets/img/search.png"></a>
                </td>
                <td>
                    如果没有供应商信息，请<a href="javascript:openPage('<?php echo base64_encode('?app_act=base/supplier/detail&app_scene=add') ?>', '?app_act=base/supplier/detail&app_scene=add', '添加供应商');" onclick="">新增</a>
                </td>
            </tr>
            ...
            <tr><td style="width:108px;">
                </td><td><button type="submit" class="button button-primary">保存</button>&nbsp;&nbsp;&nbsp;<button type="reset" class="button button-primary">重置</button> &nbsp;&nbsp;&nbsp;<input type="button" class="button button-primary"  value="返回" onclick="javascript:window.location = '?app_act=prm/goods/do_list';">
                ...
                <tr><td style="width:80px;" >
                        ...
                        <div align="left">
                            <div class="scroll" id="spec1_html">
                                <?php
                                foreach ($response['data']['goods_spec1_code'] as $k => $v) {
                                    if ($v != '' && isset($v)) {
                                        ?>
                                        <?php if ($k % 12 == 0 && $k > 0) { ?>
                                        <?php } ?>
                ...
                <tr><td><?php echo $response['goods_spec2_rename']; ?>  <input type="hidden" id="spec2_code"  value="<?php echo $response['data']['goods_spec2_str_code']; ?>" name="spec2_code" /><br>  <?php if ($response['spec_power']['spec_power'] == 1) { ?><a href="#" id = 'goods_spec2'><img src='assets/img/search.png'>点我新增</a><?php } ?>
                        ...
                        <div align="left">
                            <div class="scroll" id="spec2_html" >
                                <?php
                                foreach ($response['data']['goods_spec2_code'] as $k => $v) {
                                    if ($v != '' && isset($v)) {
                                        ?>
                                        <?php if ($k % 12 == 0 && $k > 0) { ?>
                                        <?php } ?>
  ...
  <!--  <td> 没找到规格2信息，需要<a href="javascript:PageHead_show_dialog_type('?app_act=prm/spec2/detail&app_scene=add&app_show_mode=pop', '添加规格2', {w:500,h:400},'get_spec2')" onclick="">添加规格2</a>   找到规格2，
                            ...
                            <tbody id="tiaoma">
                                <?php foreach ($response['data']['barcode'] as $k => $v) { ?>
                                    <tr><td><?php echo $v['spec1_name'] ?>（<?php echo $v['spec1_code']; ?>）</td><td ><?php echo $v['spec2_name'] ?>（<?php echo $v['spec2_code']; ?>）</td><td ><?php echo $v['sku'] ?><input  id= "<?php echo $v['spec1_code'] . '_' . $v['spec2_code'] . '_sku'; ?>"  name= "<?php echo $v['spec1_code'] . '_' . $v['spec2_code'] . '_sku'; ?>" value="<?php echo $v['sku'] ?>"  type='hidden' /></td>
                                        ...
                                        <td>
                                            <span class="shuru" >
                                                <input data-val= "<?php echo $v['spec1_code'] . '_' . $v['spec2_code'] . '_barcode'; ?>" name="<?php echo $v['spec1_code'] . '_' . $v['spec2_code'] . '_barcode'; ?>" type="text" onblur="inputbarcord(this);" style="width:98%;" value="<?php echo $v['barcode'] ?>"/>
                                            </span>
                                        </td>

                                        <td>
                                            <span class="shuru" >
                                                <input data-val= "<?php echo $v['spec1_code'] . '_' . $v['spec2_code'] . '_gb_code'; ?>" name="<?php echo $v['spec1_code'] . '_' . $v['spec2_code'] . '_gb_code'; ?>" type="text" onblur="inputgbcode(this);" style="width:98%;" value="<?php echo $v['gb_code'] ?>"/>
                                            </span>
                                        </td>
                                        ...
                                        <td >
                                            ...
                                            $sell_price_str = urlencode($spec12_code.'_sell_price');

                                            echo "<input type='text' data-val='{$spec12_code}_sell_price' name = '{$sell_price_str}' style='width:98%' onblur='inputprice(this);' value='{$sell_price}'/>";
                                            //echo $sell_price;
                                            ?>
                                        </td>
                                        <td>
                                            ...
                                            echo "<input type='text' data-val='{$spec12_code}_cost_price' name = '{$spec12_code}_cost_price' onblur='inputcostprice(this);' style='width:98%' value='{$cost_price}'/>";
                                            ?>
                                        </td>
                                        <td >
                                            <?php
                                            ...
                                            echo "<input type='text' data-val='{$spec12_code}_weight' name = '{$spec12_code}_weight' onblur='inputweight(this);' style='width:98%' value='{$weight}'/>";
                                            //echo $weight;
                                            ?>
                                        </td>
                                        <td>
                                            ...
                                            echo "<input type='text' data-val='{$spec12_code}_sku_remark' name='{$spec12_code}_sku_remark' style='width:98%' value='{$sku_remark}'/>";
                                            ?>
                                        </td>
                                    </tr>
                                <?php } ?>
                ...
                <tr><td style="width:80px;"></td><td><button type="submit" class="button button-primary">保存</button>&nbsp;&nbsp;&nbsp;<button type="reset" class="button button-primary">重置</button>&nbsp;&nbsp;&nbsp;<input type="button" class="button button-primary"  value="返回" onclick="javascript:window.location = '?app_act=prm/goods/do_list';">&nbsp;&nbsp;&nbsp;<input type="hidden" name="msg" id="msg"></td></tr>
            </table>
            <div><br>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
                <?php if (isset($response['data']['diy']) && $response['data']['diy'] === '1') { ?>
...
<script type="text/javascript">
    ...
    parent.add_spec1 = function (ids) {
        $.each(ids, function (i, val) {
            $("#spec1_html").append("<div class='spec1-code-name'><span> <input class='bui-form-field-checkbox bui-form-check-field bui-form-field' data-val='spec1_" + val[0] + "'  name='spec1[]' checked aria-disabled='false' type='checkbox' value='" + val[0] + "'/> </span><span class='spec1-name' title = '" + val[1] + "(" + val[0] + "）'>" + val[1] + "(" + val[0] + "）</span></div>");
            $("input[data-val='spec1_" + val[0] + "']").click(function () {
                spec_checked('spec1', this);
            });
            $("input[data-val='spec1_" + val[0] + "']").trigger("click");
            $("input[data-val='spec1_" + val[0] + "']").attr('checked', true);
    ...
    parent.add_spec2 = function (ids) {
        $.each(ids, function (i, val) {
            $("#spec2_html").append("<div class='spec1-code-name'><span> <input class='bui-form-field-checkbox bui-form-check-field bui-form-field' name='spec2[]' checked type='checkbox' data-val='spec2_" + val[0] + "' value='" + val[0] + "'/> </span><span class='spec1-name' title = '" + val[1] + "(" + val[0] + "）'>" + val[1] + "(" + val[0] + "）</span></div>");
            $("input[data-val='spec2_" + val[0] + "']").click(function () {
                spec_checked('spec2', this);
            });
            $("input[data-val='spec2_" + val[0] + "']").trigger("click");
            $("input[data-val='spec2_" + val[0] + "']").attr('checked', true);
        });
        spec2Check();
        sku_cost_price_auth();
    };
    ...
    form = new BUI.Form.HForm({
        ...
        callback: function (data) {
            ...
            if (data.status == 1) {
                ...
                window.location.href = "?app_act=prm/goods/detail&action=do_edit&goods_id=" + data.data + "&ES_frmId=" + ES_frmId;
                ...
                if (spec1_code != '' && spec2_code != '') {
                    for (var i = 0; i < arr_spec1_code.length; i++) {
                        ...
                        for (var j = 0; j < arr_spec2_code.length; j++) {
                            arr_barcode[k] = $("input[data-val='"+arr_spec1_code[i] + "_" + arr_spec2_code[j] + "_barcode']").val();
                            arr_gb_code[k] = $("input[data-val='"+arr_spec1_code[i] + "_" + arr_spec2_code[j] + "_gb_code']").val();
                            jiage =  $("input[data-val='"+arr_spec1_code[i] + "_" + arr_spec2_code[j] + "_sell_price']").val();
                            zhong =  $("input[data-val='"+arr_spec1_code[i] + "_" + arr_spec2_code[j] + "_weight']").val();
...
<?php } ?>
    ...
    function get_prop_data() {
        ...
        $.getJSON(url, {goods_code: goods_code,type:'goods'}, function (result) {
    ...
    var selectPopWindow_supplier = {
        dialog: null,
        callback: function (value) {
            var supplier_name = value[0]['supplier_name'];
            var supplier_code = value[0]['supplier_code'];
            $('#supplier_code').val(supplier_code);
            $('#supplier_name').val(supplier_name);
            if (selectPopWindow_supplier.dialog != null) {
                selectPopWindow_supplier.dialog.close();
            }
        }
    };
    ...
    $('#supplier_select_img').click(function () {
        selectPopWindow_supplier.dialog = new ESUI.PopSelectWindow('?app_act=common/select/supplier&is_multi=0&supplier_power=1', 'selectPopWindow_supplier.callback', {title: '选择供应商', width: 680, height: 500, ES_pFrmId: '<?php echo $request['ES_frmId']; ?>'}).show();
    });
