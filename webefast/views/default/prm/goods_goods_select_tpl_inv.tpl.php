            ...
            <form id="skuSearchForm" name="skuSearchForm">
                <table class="table table-condensed" style="width: 100%;">
                    ...
                    <tr>
                        ...
                        <input type="hidden" name="type" value="<?php if(!empty($response['type'])){ echo $response['type']; }else{ echo 'normal'; }?>"/></td>
                        <th>商品条形码</th>
    ...
    $(function () {
        ...
        BUI.use(['bui/grid', 'bui/data', 'bui/form','bui/tooltip'], function (Grid, Data, Form,Tooltip) {
            ...
            var grid = new Grid.Grid();
                ...
                url : '?app_act=prm/goods/goods_select_action_inv&is_entity=<?php echo $request['is_entity']?>'+"&record_code=<?php echo $response['record_code'];?>"+"&type=<?php echo $response['type'];?>&custom_code=<?php echo urlencode($response['custom_code']); ?>"+"&sell_return_code="+sell_return_code,
            ...
            var columns = [
                ...
                {title: '商品名称', dataIndex: 'goods_name', width: 100,'sortable':false, renderer : function(value,obj){
                    if(value.length<=25){
                        ...
                        return '<span class="grid-goods_name" data-title="'+objStr+'">'+value.substr(0,25)+'</span>';
                    }
                }},
                ...
                {title: '数量', dataIndex: 'num',width: 108,'sortable':false, renderer : function(value,obj){
                  <?php if(isset($response['allow_negative_inv'])&&$response['allow_negative_inv']==1):?>
                        return '<input type="text" class="input-small" name="num_'+obj.goods_inv_id+'"   data-rules="{number:true,min:1}"  value=""/>';
                    <?php else:?>
                    if(obj.available_mum==0){
                        return '<input type="text" class="input-small" value="" disabled="disabled"/>';
                    }else{
                       return '<input type="text" class="input-small" name="num_'+obj.goods_inv_id+'"   data-rules="{number:true,min:1,max:'+obj.available_mum+'}"  value=""/>';
                    }
                <?php endif;?>
                }},
                ...
                {title: '数量', dataIndex: 'num',width: 95,'sortable':false, renderer : function(value,obj){
                 <?php if(isset($response['allow_negative_inv'])&&$response['allow_negative_inv']==1):?>
                    ...
                    <?php else:?>
                        if(obj.available_mum<=0||obj.is_combo==1){
                            return '<input type="text" class="input-small" value="" disabled="disabled"/>';
                        }else{
                        return '<input type="text" class="input-small" name="num_'+obj.goods_inv_id+'"   data-rules="{number:true,min:1,max:'+obj.available_mum+'}"  value=""/>';
                        }
