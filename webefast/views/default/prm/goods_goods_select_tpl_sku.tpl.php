                ...
                <table class="table table-condensed" style="width: 100%;">
                    ...
                    <tr>
                        ...
                        <td>
                            ...
                            <th>商品编码/名称</th>
                        <td colspan="1"><input type="text" class="input" placeholder="编码/名称/简称" name="goods_code" id="goods_code" value="" /></td>
                        <th>商品条形码</th>
                    ...
                    <tr>
                       <td colspan="8" style='text-align:center;'>
                            <input type="hidden" value="" name="category_code" id="category_code"/>
                             <input type="hidden" value="<?php echo isset($request['lof_status'])?$request['lof_status']:'';?>" name="lof_status" id="category_code"/>
                            <input type="submit" value="查询" class="button button-primary" id="skuSearchFormSubmit"/>
                            <input type="reset" value="重置" class="button"/>
                        </td>
                    </tr>
...
<script type="text/javascript">
    ...
    var sku_store_code = '<?php echo $request['store_code']; ?>';
   var  formListeners = {'beforesubmit': []};
   var save_up;
   var page_size=10;
   var list_type = '<?php echo $request['type']?>';
   var is_entity = '<?php echo $request['is_entity']?>';
    ...
    $(function () {
       ...
       store.load({id : '0'});//加载根节点，也可以让用户点击加载
        ...
        BUI.use(['bui/grid', 'bui/data', 'bui/form','bui/tooltip'], function (Grid, Data, Form,Tooltip) {
            ...
            url : '?app_act=prm/goods/goods_select_action_sku&list_type=' + list_type + '&is_entity=' + is_entity+'&sell_return_code='+sell_return_code+"&custom_code=<?php echo $response['custom_code']; ?>",
             ...
             skuSelectorStore.load(obj);
                ...
                var columns = [
                   /* {
                    ...
                    {title: '商品编码', dataIndex: 'goods_code', width: 100},
                    ...
                    {title: '<?php echo $response['goods_spec2_rename'];?>', dataIndex: 'spec2_name', width: 80,'sortable':false},
                        {title: '批次号', dataIndex: 'lof_no', width: 108,'sortable':false},
                    {title : '生产日期',dataIndex :'production_date', width: 108,'sortable':false},
                    <?php if($request['type'] != 'crm_goods') {?>
					{title: '数量', dataIndex: 'num',width: 108,'sortable':false, renderer : function(value,obj){
                            return '<input type="text" class="input-small" name="num_'+obj.goods_inv_id+'"   data-rules="{number:true,min:1}"  value=""/>';
                    }},
                    <?php }?>
                    <?php if($request['type'] == 'crm_goods'){?>
                        {title: '同步比例%', dataIndex: 'sync_ratio',width:108,'sortable':false, renderer : function(value,obj){
                            return '<input type="text" class="input-small" name="sync_'+obj.goods_inv_id+'"   data-rules="{number:true,min:1}"  value=""/>';
                        }},
                    <?php }?>
                    ...
                    {title: 'spec1_code', dataIndex: 'spec1_code', visible : false,width: 50,'sortable':false},
                    {title: '<?php echo $response['goods_spec1_rename'];?>', dataIndex: 'spec1_name', width: 108,'sortable':false},
                    {title: 'spec2_code', dataIndex: 'spec2_code', visible : false, width: 50},
                    {title: '<?php echo $response['goods_spec2_rename'];?>', dataIndex: 'spec2_name', width: 108,'sortable':false},
                    <?php if($request['type'] != 'crm_goods') {?>
                        {title: '数量', dataIndex: 'num',width: 108,'sortable':false, renderer : function(value,obj){
                        <?php if(!empty($response['type']) && $response['type'] === 'minus'){?>
                            return '<input type="text" class="input-small" name="num_'+obj.goods_inv_id+'"   data-rules="{number:true,min:1}"  value=""/>';
                        <?php }else{ ?>
                    ...
                    <?php }?>
                    {title: '商品条形码', dataIndex: 'barcode', width: 140,'sortable':false}
