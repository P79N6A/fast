    ...
    <div class="row form-actions actions-bar">
        <div class="span13 offset3 ">
            <?php if($response['lof_status']==1){?>            
              <a class="button" target="_blank" style="float: left;" href="<?php echo get_excel_url("store_out_record_lof.xlsx",1) ?>">模版下载</a>
            <?php }else{ ?>
              <a class="button" target="_blank" style="float: left;" href="<?php echo get_excel_url("store_out_record.xlsx",1) ?>">模版下载</a>
            <?php } ?> 
...
// 	            minSize: [1, '文件最小不能小于1k!'],
	    ...
	    var uploader = new Uploader.Uploader({
	          ...
	          success: function(result){
                        if (confirm('确定要导入数据吗')) {
                            var url = result.url;
                            var r = '?app_act=wbm/store_out_record/import_goods_upload&is_lof=<?php echo $response['lof_status']?>&app_fmt=json&id='+id;
                            var params = {"url": url};
                            var msg = '';
