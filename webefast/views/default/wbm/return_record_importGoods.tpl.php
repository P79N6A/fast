...
<div class="upload1">
    <div class="row form-actions actions-bar">
	  ...
	  <?php if($response['lof_status']==1){?>            
            <a class="button" style="float: left;" target="_blank" href="<?php echo get_excel_url("wbm_return_record_lof.xlsx",1) ?>">模版下载</a>
          <?php }else{ ?>
            <a class="button" style="float: left;" target="_blank" href="<?php echo get_excel_url("wbm_return_record.xlsx",1) ?>">模版下载</a>
          <?php } ?>
	    ...
	    */
		...
		var uploader = new Uploader.Uploader({
			...
			success: function(result){
                            if (confirm('确定要导入数据吗')) {
                            var url = result.url;
                            var r = '?app_act=wbm/return_record/import_goods_upload&app_fmt=json&is_lof=<?php echo $response['lof_status']?>&id='+id;
                            var params = {"url": url};
                            var msg = '';
