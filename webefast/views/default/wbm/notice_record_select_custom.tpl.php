...
<div class="row">
    <div class="pull-left" style="width: 95%;">
        <div class="well" style="margin-left: 1em;">
            <form id="skuSearchForm" name="skuSearchForm" >
                <div style="margin: 10px;">
                    ...
                    <input type="text" class="input" placeholder="分销商名称/代码" name="custom_name"/>
        ...
        <?php if($request['list_type'] == 'fx_goods_list') { ?>
            <div><span style="color:red">友情提示：指定分销商后，只有指定分销商才允许查看该商品</span></div>
        <?php } ?>
    </div>
</div>
    ...
    $(function () {
        ...
        BUI.use(['bui/grid', 'bui/data', 'bui/form', 'bui/tooltip'], function (Grid, Data, Form, Tooltip) {
            ...
            skuSelectorStore = new Data.Store({
                url: '?app_act=wbm/notice_record/select_action&app_fmt=json',
            ...
            //渲染结果列表或刷新结果列表
            var columns = [
                {title: '分销商代码', dataIndex: 'custom_code', width: '50%', 'sortable': false},
                {title: '分销商名称', dataIndex: 'custom_name', width: '50%', 'sortable': false},
            ];
