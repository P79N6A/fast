...
<input type="hidden" id="profession_type" value="3" />
...
<ul class="nav-tabs oms_tabs">
    ...
    <li class="active"><a href="#" onClick="do_page('brand_list');" >品牌</a></li>
 ...
 <div class="row-fluid msg" > <?php if($response['power'] == '1'){ ?> 已启用品牌权限，停用请点击这里<a href="#" onClick="do_set_active_shenhe('brand_power','disable');"> <font color="#0000FF ">停用</font></a> <?php }else{ ?>  未启用品牌权限，只有启用后才允许配置，启用请猛击这里<a href="#" onClick="do_set_active_shenhe('brand_power','enable');"> <font color="#0000FF ">启用</font></a> <?php } ?></div>
<div class="row-fluid" <?php if($response['power'] == '1'){ ?> style="display:block;" <?php }else{ ?>style="display:none;" <?php } ?>>
  <div class="span12">
  	<div class="div_title">可选品牌列表</div>
	<?php
	render_control ( 'DataTable', 'DataTable2', array (
			...
			'dataset' => 'sys/RoleProfessionModel::get_brand_list_noset',
			'queryBy' => 'searchForm',
			'idField' => 'brand_id',
  ...
  <div class="span11">
  	<div class="div_title">已选品牌列表</div>  
	<?php
	render_control ( 'DataTable', 'DataTable3', array (
			...
			'dataset' => 'sys/RoleProfessionModel::get_brand_list',
