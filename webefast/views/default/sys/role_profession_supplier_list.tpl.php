...
<input type="hidden" id="profession_type" value="4" />
...
<ul class="nav-tabs oms_tabs">
    ...
    <li class="active"><a href="#" onClick="do_page('supplier_list');" >供应商</a></li>
    <?php endif; ?>
...
<div class="row-fluid msg" > <?php if ($response['power'] == '1') { ?> 已启用供应商权限，停用请点击这里<a href="#" onClick="do_set_active_shenhe('supplier_power', 'disable');"> <font color="#0000FF ">停用</font></a> <?php } else { ?>  未启用供应商权限，只有启用后才允许配置，启用请猛击这里<a href="#" onClick="do_set_active_shenhe('supplier_power', 'enable');"> <font color="#0000FF ">启用</font></a> <?php } ?></div>
<div class="row-fluid" <?php if ($response['power'] == '1') { ?> style="display:block;" <?php } else { ?>style="display:none;" <?php } ?>>
    <div class="span12">
        <div class="div_title">可选供应商列表</div>
<?php
render_control('DataTable', 'DataTable2', array(
    ...
    'dataset' => 'sys/RoleProfessionModel::get_supplier_list_noset',
    'queryBy' => 'searchForm',
    'idField' => 'supplier_id',
    //'init'=>'nodata',
...
?>
    ...
    <div class="span11">
        <div class="div_title">已选供应商列表</div>  
<?php
render_control('DataTable', 'DataTable3', array(
    ...
    'dataset' => 'sys/RoleProfessionModel::get_supplier_list',
    'idField' => 'sys_role_profession_id',
    //'init'=>'nodata',
    'params' => array('filter' => array('role_code' => $request['role_code'])),
    'CheckSelection' => true,
));
