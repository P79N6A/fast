						...
						<div class='submenu' id="div_<?php echo $conf_key ?>">
							<?php if ('grid' == $conf_val['type']) { ?>
								<div class="subhead" id="<?php echo $conf_key; ?>">
									...
									<?php if ($conf_key == 'barcode_area'){?>
										<input type="button" value="配置" onclick="dlg_table_setting('div_table_<?php echo $conf_key ?>',1)"/>
									<?php } else {?>
										<input type="button" value="配置" onclick="dlg_table_setting('div_table_<?php echo $conf_key ?>')"/>
									<?php }?>
									<?php if ($conf_key == 'barcode_area'){?>
										<input type="button" value="条码配置" onclick="dlg_barcode_setting()"/>
									<?php }?>
		...
		<td valign="top" width="100%">
			<div style="width: 100%">
				<div class="main_print_nr" id="danju_print_content"
				     style="width:<?php echo $response['danju_print_data']['template_page_width']-2; ?>mm;margin:0 auto;">
			...
			<input type="button" value="刷新" onclick="refresh()" class="button button-primary"/>
		</td>
	</tr>
</table>
...
<?php
	...
	$barcode_height = 8;
    $barcode_width = '';
	$barcode_top_offset = $barcode_left_offset = 1;

	$codeStyle = '128A';

	$barcode_space_between = 6;

	if (not_null($response['danju_print_data']['extend_attr'])) {
		$extend_attr = object_to_array(json_decode($response['danju_print_data']['extend_attr']));
		if (not_null($extend_attr['barcode_height'])) {
			$barcode_height = $extend_attr['barcode_height'];
		}

		if (not_null($extend_attr['barcode_width'])) {
			$barcode_width = $extend_attr['barcode_width'];
		}

		if (not_null($extend_attr['barcode_top_offset'])) {
			$barcode_top_offset = $extend_attr['barcode_top_offset'];
		}

		if (not_null($extend_attr['barcode_left_offset'])) {
			$barcode_left_offset = $extend_attr['barcode_left_offset'];
		}

		if (not_null($extend_attr['codeStyle'])) {
			$codeStyle = $extend_attr['codeStyle'];
		}

		if (not_null($extend_attr['barcode_space_between'])){
			$barcode_space_between = $extend_attr['barcode_space_between'];
		}
		if (not_null($extend_attr['barcode_col']))
			$barcode_col = $extend_attr['barcode_col'];
			
		if (not_null($extend_attr['barcode_col_width']))
			$barcode_col_width = $extend_attr['barcode_col_width'];				
	}
?>
<div id="dlg_barcode_setting" style="display: none">
	<table width="100%" class="table">
		<tr>
			<td style="text-align: right">条码高度：</td>
			<td><input style="margin-left: 10px;" type="text" value="<?php echo $barcode_height; ?>" id="barcode_height"/>mm</td>
		</tr>
		<tr>
			<td style="text-align: right">条码宽度：</td>
			<td><input style="margin-left: 10px;" type="text" value="<?php echo $barcode_width; ?>" id="barcode_width"/>mm</td>
		</tr>

		<tr>
			<td style="text-align: right">上偏移量：</td>
			<td><input style="margin-left: 10px;" type="text" value="<?php echo $barcode_top_offset; ?>" id="barcode_top_offset"/>mm</td>
		</tr>

		<tr>
			<td style="text-align: right">左偏移量：</td>
			<td><input style="margin-left: 10px;" type="text" value="<?php echo $barcode_left_offset; ?>" id="barcode_left_offset"/>mm</td>
		</tr>
		<tr>
			<td style="text-align: right">条码码制：</td>
			<td>
				<select id="codeStyle" style="margin-left: 10px;">
					<option value="128A" <?php if ($codeStyle == '128A') echo 'selected'?> >128A</option>
					<option value="128B" <?php if ($codeStyle == '128B') echo 'selected'?> >128B</option>
					<option value="128C" <?php if ($codeStyle == '128C') echo 'selected'?> >128C</option>
					<option value="EAN128A" <?php if ($codeStyle == 'EAN128A') echo 'selected'?> >EAN128A</option>
					<option value="EAN128B" <?php if ($codeStyle == 'EAN128B') echo 'selected'?> >EAN128B</option>
					<option value="EAN128C" <?php if ($codeStyle == 'EAN128C') echo 'selected'?> >EAN128C</option>
					<option value="Code39" <?php if ($codeStyle == 'Code39') echo 'selected'?> >Code39</option>
				</select>
			</td>
		</tr>

		<tr>
			<td style="text-align: right">条码与内容间距：</td>
			<td><input style="margin-left: 10px;" type="text" value="<?php echo $barcode_space_between; ?>" id="barcode_space_between"/>mm</td>
		</tr>
		<tr>
			<td style="text-align: right">横排多列的栏数：</td>
			<td><input style="margin-left: 10px;" type="text" value="<?php echo $barcode_col; ?>" id="barcode_col"/></td>
		</tr>
		<tr>
			<td style="text-align: right">横排多列的栏宽：</td>
			<td><input style="margin-left: 10px;" type="text" value="<?php echo $barcode_col_width; ?>" id="barcode_col_width"/>mm</td>
		</tr>		
	</table>
</div>
...
<!--		<tr>-->
<!--			<td style="width:30%;text-align: center;">DPI:</td>-->
<!--			<td>-->
<!--				<input style="margin-left: 10px;" type="text" id="dpi" value="--><?php //if (not_null($extend_attr['dpi'])) echo $extend_attr['dpi']; else echo ''; ?><!--"/>-->
<!--			</td>-->
<!--		</tr>-->
	</table>
</div>
 ...
 * @param div_id
 */
function dlg_table_setting(div_id,is_barcode) {
	//2014-09-23
	$('#dlg_table_setting').find('#_th_table_font_family').show();
	...
	//2014-09-23
	if(is_barcode) {
		$('#dlg_table_setting').find('#_th_table_font_family').hide();
	}

	jQuery('#dlg_table_setting').dialog({
		modal: true,
		resizable: false, width: 400, height: 300, autoOpen: true,
		title: "表格配置",
		buttons: {
 ...
 * @param div_id
 */
function dlg_barcode_setting() {

	jQuery('#dlg_barcode_setting').dialog({
		modal: true,
		resizable: false, width: 400, height: 340, autoOpen: true,
		title: "条码配置",
		buttons: {
			"取消": function () {
				$(this).dialog('close');
			...
			"确定": function () {
				var _barcode_height = $('#barcode_height').val();
				if(_barcode_height && !$.isNumeric(_barcode_height)) {
					alert('请输入数字');
					return;
				}

				var _barcode_width = $('#barcode_width').val();
				if(_barcode_width && !$.isNumeric(_barcode_width)) {
					alert('请输入数字');
					return;
				}

				var _barcode_top_offset = $('#barcode_top_offset').val();
				if(_barcode_top_offset && !$.isNumeric(_barcode_top_offset)) {
					alert('请输入数字');
					return;
				}

				var _barcode_left_offset = $('#barcode_left_offset').val();
				if(_barcode_left_offset && !$.isNumeric(_barcode_left_offset)) {
					alert('请输入数字');
					return;
				}

				var _codeStyle = $("#codeStyle").val();

				var _barcode_space_between = $('#barcode_space_between').val();
				if(_barcode_space_between && !$.isNumeric(_barcode_space_between)) {
					alert('请输入数字');
					return;
				}
				var _barcode_col = $('#barcode_col').val();
				if(_barcode_col && !$.isNumeric(_barcode_col)) {
					alert('请输入数字');
					return;
				}	
				var _barcode_col_width = $('#barcode_col_width').val();
				if(_barcode_col_width && !$.isNumeric(_barcode_col_width)) {
					alert('请输入数字');
					return;
				}							
				
				var url = "?app_act=sys/danju_print/do_save_barcode_setting&app_fmt=json&app_page=null";
				var params = {
					'danju_print_code': '<?php echo $response['danju_print_code']?>',
					'barcode_height': _barcode_height,
					'barcode_width': _barcode_width,
					'barcode_top_offset':_barcode_top_offset,
					'barcode_left_offset':_barcode_left_offset,
					'codeStyle' : _codeStyle,
					'barcode_space_between' : _barcode_space_between,
					'barcode_col' : _barcode_col,
					'barcode_col_width' : _barcode_col_width,
					'shop_code':shop_code
				}
	...
	jQuery('#dlg_list_table_setting').dialog({
		...
		buttons: {
			...
			"确定": function () {
				...
				save_table_setting(div_id,table_line_style, table_td_height,table_th_height);
				$(this).dialog('close');
			}
		}
	});
