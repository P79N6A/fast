    ...
    <div class="row form-actions actions-bar">
        <div class="offset3 ">
            <?php if ($response['lof_status'] == 1) { ?>            
                <a class="button" style="float: left;" target="_blank" href="?app_act=sys/file/get_file&type=1&name=fx_purchaser_record_lof.xlsx">模版下载</a>
            <?php } else { ?>
                <a class="button" style="float: left;" target="_blank" href="<?php echo get_excel_url("fx_purchaser_reutrn_record_goods.xlsx",1) ?>">模版下载</a>
            <?php } ?>	
...
<script type="text/javascript">
    ...
    BUI.use('bui/uploader', function (Uploader) {
        ...
        var uploader = new Uploader.Uploader({
            ...
            success: function (result) {
                if (confirm('确定要导入数据吗')) {
                    var url = result.url;
                    var r = '?app_act=fx/purchase_return_record/import_goods&app_fmt=json&is_lof=<?php echo $response['lof_status'] ?>&id=' + id;
                    var params = {"url": url};
                    var msg = '';
    ...
    $(document).ready(function () {
        $("#submit_import_goods").click(function () {
            ...
            var r = '?app_act=pur/purchase_record/import_goods&app_fmt=json&is_lof=<?php echo $response['lof_status'] ?>&id=' + id;
            var params = {"url": url};
            var msg = '';
            $.post(r, params, function (data) {
                var type = data.status == 1 ? 'success' : 'error';
