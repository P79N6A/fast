...
/*$my_fields = array(
            ...
            array('title' => '数量', 'type' => 'input', 'field' => 'num'),
            array('title' => '吊牌价', 'type' => 'label', 'field' => 'goods_price'),
            array('title' => '实际应收款', 'type' => 'input', 'field' => 'avg_money'),
        );
if (load_model('sys/PrivilegeModel')->check_priv('oms/return_opt/change_goods_del')) {
	$oprate_del = '<a onclick=javascript:if(confirm("确定要删除吗？")){delete_change_detail("change_goods",this);}>删除</a>';
}
if (load_model('sys/PrivilegeModel')->check_priv('oms/return_opt/change_goods_change')) {
	$oprate_change = '<a onclick=javascript:goods_change_detail(this);>改款</a>';
}
$oprate =$oprate_del.'&nbsp;&nbsp;&nbsp;'.$oprate_change; 
if($app['scene']=='edit'){
	$my_fields[] = array('title' => '操作', 'type' => 'label', 'field'=>'operate',
			'value' => $oprate);
}*/ 
/*   
if($app['scene']=='edit'){
    $my_fields[] = array('title' => '操作', 'type' => 'label', 'field'=>'operate', 
    		'value' => '<a onclick=javascript:if(confirm("确定要删除吗？")){delete_change_detail("change_goods",this);}>删除</a>&nbsp;&nbsp;&nbsp;<a onclick=javascript:goods_change_detail(this);>改款</a>');
}*/

/*render_control('TableForm', 'change_goods_form', array(
    'conf' => array(
        'fields' => $my_fields,
    ),
    'act_edit'=>'fx/sell_return/do_change_goods_edit&app_fmt=json',
    'buttons' => array(),
    'data' => @$response['data']['change_detail_list'],
));*/
?>
...
<table cellspacing="0" class="table table-bordered">
    ...
    <tbody>
    <?php 
    foreach($response['data']['change_detail_list'] as $key=>$detail){?>
...
function chg_detail_save(id){
    ...
    //console.log(params);
    $.post("?app_fmt=json&app_act=fx/sell_return/save_component&type=change_goods&sell_return_code="+sell_return_code, params, function(data){
...
function chg_detail_delete(id){
    ...
    BUI.Message.Confirm(msg,function(){
        ...
        $.post(
            "?app_act=fx/sell_return/delete_change_detail_by_id&app_fmt=json", 
            params, 
            function(data){
                if(data.status == 1){
                    component('change_goods', "view");
    ...
    callback: function (value, id, code, name) {
       ...
       if(num.match(re)==null) 
        ...
        return;
             ...
             $.ajax({
                    type: "GET",
                    url: "?app_act=fx/sell_return/opt_change_detail",
                    //async: false,
                    data: {sku:value.sku,
                        num:value.num,
