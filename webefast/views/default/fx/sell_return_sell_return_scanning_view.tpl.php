...
<html xmlns="http://www.w3.org/1999/xhtml">
    ...
    <body style="overflow-x:hidden; background:#f6f6f6;">
                    ...
                    <tbody>
                        <?php foreach ($response['mx'] as $sub_mx) { ?>
                            <tr>
                                ...
                                <td class="smsl" id="sku_num_<?php echo $sub_mx['barcode']; ?>"><?php echo "<span>{$sub_mx['recv_num']}</span>"; ?></td>
                            </tr>
                        </tbody>
                    <?php } ?>
                </table>
        ...
        <script type="text/javascript">
            ...
            var detail = <?php echo json_encode($response['detail_key']); ?>;
            ...
            function update_detail_num(scan_barcode) {
                ...
                if (type == 'package' && sell_return_code == '') {
                    url = "?app_act=fx/sell_return/scan_barcode_no_return_code";
                } else {
                    url = "?app_act=fx/sell_return/scan_barcode&type=" + type;
                }
...
//                }
                ...
                var scaned_num = $("#sku_num_" + scan_barcode).text();
                $("#sku_num_" + scan_barcode).html(parseInt(scaned_num)+1);
                detail[scan_barcode]['num'] = parseInt(scaned_num)+1;
                $("#scan_barcode").val("");
            ...
            function verify_add_barcode(result) {
                if (result.status == 3) {
                    ...
                    html = "<tr><td>" + data.goods_name + "</td><td>" + data.goods_code + "</td><td>" + data.spec1_name + "</td><td>" + data.spec2_name + "</td><td class='barcode'>" + data.barcode + "</td><td class='note_num'>" + data.apply_num + "</td><td  class='smsl' id='sku_num_" + data.barcode + "'><span >" + data.num + "</span></td></tr>";
                    $("#sku_tbl").append(html);
                    detail[data.barcode] = {};
                    detail[data.barcode]['note_num'] = data.apply_num;
                    detail[data.barcode]['num'] = data.num;
                    $("#total_scan_sl").html(parseInt($("#total_scan_sl").html()) + 1);
                    $("#diff_num").html(parseInt($("#diff_num").html()) - 1);
                }
            }
            function barcode_check() {
                var iSku = $("#scan_barcode").val();
                scan_barcode_ret = update_detail_num(iSku);
            }
   
//            function insert_unique_barcode(unique_code) {
//                var url = "?app_act=fx/sell_return/insert_unique_barcode";
            ...
            function submit_it(type) {
                ...
                $.post("?app_act=fx/sell_return/opt_return_shipping", params, function (data) {
                    if (data.status != 1) {
                        ...
                        if (unique_status == 1 && parseInt($("#total_sl").html()) == parseInt($("#total_scan_sl").html())) {
