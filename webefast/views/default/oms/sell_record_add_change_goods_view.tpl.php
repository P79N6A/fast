...
<div class="panel">
    ...
    <div style="color:red;">
        温馨提示：<br/>
        保存成功，换货商品会直接覆盖原来的商品，请注意商品数量和金额。
    </div>
</div>
...
<script>
    ...
    $(function () {
        ...
        BUI.use(['bui/grid', 'bui/data', 'bui/form','bui/tooltip'], function (Grid, Data, Form,Tooltip) {
            ...
            skuSelectorStore = new Data.Store({
                url : '?app_act=oms/sell_record/search_change_goods',
    ...
    select_change_goods(1)
});

function select_change_goods(val){
	var select_goods;
        if(val == 1){
	...
	if (select_goods == ""){
        ...
        var lof_status = '<?php echo  $response['cur_goods']['lof_status'];?>'
	var obj = {goods_multi:select_goods,store_code:store_code,lof_status:lof_status};
...
function save_change_goods(){
	...
	var sell_record_code = '<?php echo $response['cur_goods']['sell_record_code'];?>';
        if($("#select_goods").val()){
            goods_code = $("#select_goods").val();
        }else{
            goods_code = '<?php echo $response['cur_goods']['goods_code'];?>';
        };
        var deal_code = '<?php echo $response['cur_goods']['deal_code'];?>';
        var sell_record_detail_id = '<?php echo $response['cur_goods']['sell_record_detail_id'];?>';
        var avg_money = '<?php echo $response['cur_goods']['avg_money'];?>';
        var barcode = '<?php echo $response['cur_goods']['barcode'];?>'
        var trade_price = '<?php echo $response['cur_goods']['trade_price'];?>'
        var fx_amount = '<?php echo $response['cur_goods']['fx_amount'];?>'
	var url = "?app_fmt=json&app_act=oms/sell_record/opt_change_detail";
	var data = {sku:change_sku,deal_code:deal_code,sell_record_code:sell_record_code,num:num,goods_code:goods_code,sell_record_detail_id:sell_record_detail_id,avg_money:avg_money,barcode:barcode,trade_price:trade_price,fx_amount:fx_amount}
