    ...
    <div class="panel-body" id="panel_order">
		...
		*/
        ...
        render_control('FormTable', 'form1', array(
            ...
            'col'=>'3',
            'act_edit' => "oms/sell_record/create_return",
            'buttons' => array(),
...
$(function(){
    ...
    $("#btn_create_return").click(function(){
		...
		//console.log(data);
		//return;
        $.post("?app_act=oms/sell_record/create_return&app_fmt=json",data,function(ret){
            if(ret.status!=1){
                BUI.Message.Alert(ret.message,'error');
            }else{
                location.href= "?app_act=oms/sell_return/after_service_detail&sell_return_code="+ret.data+"&ES_frmId="+ES_frmId;
            }
        },'json');
    });
    //关闭按钮
 ...
 $("input[name='return_num']").change(function(){
	 ...
	 var avg_money = mx_json['id_'+sell_record_detail_id]['avg_money'];
        ...
        cur_je = Math.round(cur_je*100)/100;
            ...
            $("#avg_money_"+sell_record_detail_id).val(avg_money);
            get_refund_total_fee();
        }
 });
