...
<?php
if ($response['power']['deliver_record_import']) {
    $links[] = array('url' => 'oms/sell_record/history_import_trade', 'title' => '已发货单导入', 'is_pop' => false);
}
if ($response['power']['sell_record_import']) {
    $links[] = array('url' => 'oms/sell_record/import_trade', 'title' => '订单导入', 'is_pop' => false);   
}
            ...
            <table cellspacing="0" class="table table-bordered">
                <tbody>
                    <tr>
                        ...
                        <td>手机：</td>
                        <td><input id="receiver_mobile" type="text" value="" name="receiver_mobile"></td>
                        <td>固定电话：</td>
                        <td><input id="receiver_phone" type="text" value="" name="receiver_phone"</td>
                    </tr>
                    <tr>
                        <td>收货地址：</td>
...
<?php } ?>
    ...
    <div style="text-align: center;">
        <?php if ($response['power']['next_step']){ ?>
...
<script>
    ...
    $('#buyer_code_select_pop,#buyer_code_select_img').click(function () {
        ...
        var str = '';
        if(login_type == 2) {
            str = '&shop_code=' + shop_code;
        }
        selectPopWindowshelf_code.dialog = new ESUI.PopSelectWindow('?app_act=common/select/customer' + str, 'selectPopWindowshelf_code.callback', {title: '选择会员', width: 900, height: 500, ES_pFrmId: '<?php echo $request['ES_frmId']; ?>'}).show();
    });
...
<script type="text/javascript">
    ...
    BUI.use('bui/form', function (Form) {
        ...
        form1.on('beforesubmit', function () {
            ...
            //收货人地址不能超过150个字符
            var length = $("#receiver_addr").val().length;
            if (length > 100) {
                BUI.Message.Alert('收货人地址不允许超过100个字','error');
                return false;
            }
            $.ajax({
                type: "POST",
