...
<?php
$title = "订单详情-<font color='red'>订单号：" . $response['record']['sell_record_code'] . "</font>";
...
<div class="panel_wrap">
    ...
    <div class="edit_goods_info" id="edit_goods_info">
        <div class="panel-header clearfix" style="padding:0 0 3px;">
            <div class="pull-right">
                ...
                <button class="button button-small" id="btn_add_goods" onclick="btn_add_goods(1);"  <?php if ($response['record']['order_status'] != 0 || ($response['record']['is_fenxiao'] == 2 && $response['record']['is_fx_settlement'] == 1)) { ?>
                            disabled="disabled" 
                        <?php } ?>><i class="icon-plus"></i>新增商品</button>
                        <?php if ($response['record']['is_fenxiao'] != 2) { ?>
                    <button class="button button-small" id="btn_add_combo_goods" onclick="btn_add_combo_goods(1);"  <?php if ($response['record']['order_status'] != 0) { ?>
                                disabled="disabled" 
    ...
    <div class="panel">
        <div class="panel-header clearfix">
            ...
            <div class="pull-right">
                ...
                <button class="button button-small" onclick="btn_add_goods(2);" id="btn_add_detail"><i class="icon-plus"></i>新增商品</button>

                <?php if ($response['record']['is_fenxiao'] != 2) { ?>
                    <button class="button button-small" onclick="btn_add_combo_goods(2);" id="btn_add_combo_detail"><i class="icon-plus"></i>新增套餐</button>
                <?php } ?>
    ...
    <div class="clearfix" id="tools" style="text-align: center;position: fixed;bottom: 0px;left:-100%;width:100%;">
        <p class="p_btns">
            <button class="button button-primary" id="btn_opt_lock">请先锁定</button>
        ...
        <p class="p_btns">
            <button class="button button-primary" id="btn_opt_label">打标</button>
            ...
            <button class="button button-primary" id="btn_opt_communicate_log">沟通日志</button>
    ...
    <script>
        ...
        var record_status = "<?php echo $response['record']['order_status'] ?>";
        ...
        var componentBtns = ['baseinfo', 'shipping', 'money'];
        var opts = [
        ...
        var btns = {
            'edit_baseinfo': 0, 'edit_shipping': 0, 'edit_money': 0, 'add_detail': 0, 'edit_detail': 0,
            ...
            'opt_problem': 0, 'opt_unproblem': 0, 'opt_pending': 0, 'opt_unpending': 0, 'opt_create_return': 0, 'opt_cancel': 0, 'opt_copy': 0, 'opt_force_unlock': 0, 'opt_intercept': 0, 'add_combo_detail': 0,
            'opt_replenish':0
        };
        ...
        //新增明细按钮
        function btn_add_goods(click_type) {
        ...
        function btn_init_opt_copy() {
            $("#btn_opt_copy").click(function () {
                ...
                $.post("?app_act=oms/sell_record/opt", params, function (data) {
                    if (data.status == 1) {
                        location.href = "?app_act=oms/sell_record/view&sell_record_code=" + data.data + "&ref=do";
                    } else {
        ...
        function btn_init_component_save(id) {
            $("#btn_save_" + id).click(function () {
                ...
                component(id, "view");
                if (id == 'shipping') {
                    component('base_order_info,shipping_info,inv_info,detail,action,invoice_info', "view");
                    /*
        ...
        //检查所有按钮权限
        function btn_check() {
            var params = {"sell_record_code": sell_record_code, "fields": btns};
        ...
        function btn_init_opt_create_return() {
            $("#btn_opt_create_return").live("click", function () {
                openPage('<?php echo base64_encode('?app_act=oms/sell_record/create_return_form&app_scene=edit&sell_record_code='); ?>' + sell_record_code, '?app_act=oms/sell_record/create_return_form&app_scene=edit&sell_record_code=' + sell_record_code, '生成退单');
            });
        }


        function addgoods(obj, type, click_type) {
            var deal_code = (click_type == 2) ? $("#goods_deal_code_2").val() : $("#goods_deal_code").val();
            ...
            $.post('?app_act=oms/sell_record/opt_new_multi_detail&sell_record_code=' + sell_record_code + '&store_code=' + store_code, {data: select_data, deal_code: deal_code}, function (result) {
        ...
        $("#btn_opt_communicate_log").click(function () {
            new ESUI.PopWindow("?app_act=oms/sell_record/communicate_log&sell_record_code=" + sell_record_code, {
                title: "沟通日志",
                width: 450,
                height: 350,
                onBeforeClosed: function () {
        ...
        $("#btn_opt_label").click(function () {
            new ESUI.PopWindow("?app_act=oms/sell_record/do_label&batch=<?php echo urlencode("批量操作"); ?>&sell_record_code=" + sell_record_code, {
                title: "打标签",
                width: 500,
                height: 300,
                onBeforeClosed: function () {
...
//新增明细按钮
        function btn_add_combo_goods(click_type) {
            ...
            var buttons = [
                {
                    ...
                    elCls: 'button button-primary',
                    handler: function () {
                        addgoods_combo(this, 1, click_type);
                        top.save_up();
                    }
                },
                {
                    ...
                    elCls: 'button button-primary',
                    handler: function () {
                        addgoods_combo(this, 0, click_type);
                    }
                }, {
                    text: '取消',
                    elCls: 'button',
        ...
        function addgoods_combo(obj, type, click_type) {
            var deal_code = (click_type == 2) ? $("#goods_deal_code_2").val() : $("#goods_deal_code").val();
            var data = top.skuSelectorStore.getResult();
            var select_data = {};
            var di = 0;
            ...
            $.post('?app_act=oms/sell_record/opt_new_combo_detail&sell_record_code=' + sell_record_code + '&store_code=' + store_code, {data: select_data, deal_code: deal_code}, function (result) {
                if (true != result.status) {
                    //添加失败
                    top.BUI.Message.Alert(result.message, function () {
                        $("div .panel").hide();
