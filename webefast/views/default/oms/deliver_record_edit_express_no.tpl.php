<form id="form1" method="post" action="?app_act=oms/deliver_record/edit_express_no_action" tabindex="0" style="outline: none;">
    ...
    <table cellspacing="0" class="table table-bordered" id="record_list">
        ...
        <?php foreach($response['deliver_record_list'] as $k=>$v){ ?>
            <tr id="row_<?php echo $k;?>" class="<?php echo $v['express_code']?>">
                ...
                <td>
                    <input type="text" class="wuliu_no" name="express_no[<?php echo $v['deliver_record_id']?>]" id="express_no_<?php echo $v['deliver_record_id']?>" value="<?php echo $v['express_no']?>">
    ...
    <div class="clearfix" style="text-align: center;">
    	<input type="button" class="button button-primary" id="btn_save"  value = "确定">
...
<script>
    ...
    $(document).ready(function(){
    	$("#btn_save").click(function(){
           	...
           	$.post('<?php echo get_app_url('oms/deliver_record/edit_express_no_action');?>', data, function(data){
    	...
    	$(".wuliu_no").bind('keypress',function(event){
    	    if(event.keyCode == "13")
    	    {
    	       
    	        var express_no = $(this).val();
    	        var i = $(this).parent("td").find("button").val();
    	        i = parseInt(i);
    	       
    	        next_pipei(express_no,i);
    	    }
    	});
    ...
    //向下匹配
	function next_pipei(express_no,i){
		  var params = {
		      "express_code": $("#express_code").val(),
