<form id="form1" method="post" action="?app_act=oms/sell_record/edit_express_no_action" tabindex="0" style="outline: none;">
...
<table cellspacing="0" class="table table-bordered" id="record_list">
    ...
    <tbody>
    <?php foreach($response['sell_record_list'] as $k=>$v){ ?>
        <tr id="row_<?php echo $k;?>" class="<?php echo $v['express_code']?>">
            <td><?php echo $v['record_code']?></td>
            <td>
                <input type="text" name="express_no[<?php echo $v['sell_record_id']?>]" id="express_no_<?php echo $v['sell_record_id']?>" value="<?php echo $v['express_no']?>">
...
<script>
    var rows = <?php echo count($response['sell_record_list']);?>;
