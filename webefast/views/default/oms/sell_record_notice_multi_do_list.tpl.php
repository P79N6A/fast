...
render_control('SearchForm', 'searchForm', array(
    ...
    'fields' => array(
        ...
        array(
            ...
            'child' => array(
                array('title' => 'start','type' => 'input','field' => 'record_goods_num_start','class'=>'input-small'),
                array('pre_title' => '~','type' => 'input','field' => 'record_goods_num_end','class'=>'input-small'),
            )
        ),
        array(
            'label' => '订单价格',
...
//        	'field' => 'daterange3',
//        	'child' => array(
//        		array('title' => 'start', 'type' => 'time', 'field' => 'record_time_start','class'=>'input-small'),
//        		array('pre_title' => '~', 'type' => 'time', 'field' => 'record_time_end','class'=>'input-small', 'remark' => ''),
//        	)
//        ),
//        array(
//        	'label' => '付款时间',
...
<div class="row" >
    <button type="button" class="button" style="color:#1695ca" >多SKU分析</button>
    <button type="button" onclick = "do_page('single');"  class="button" >单SKU分析</button>
</div>
<div class="row" >
    <div style="float:right; right: 200px;">
        <input   id="sku_all_num" type="checkbox" name="sku_all_num" checked="checked" />  商品SKU数量匹配
    </div>
 </div>
...
<input id="sku_num" type="hidden" name="sku_num"  value="10"/>
...
render_control('DataTable', 'table', array(
    'conf' => array(
        'list' => array(
            array(
                'type' => 'text',
                'show' => 1,
                'title' => '条码 <img height="23" width="23" data-align="top-right" class="tip" src="assets/images/tip.png" title="勾选商品SKU数量匹配：数量/商品编码/'.$response['goods_spec1_rename'].'/'.$response['goods_spec2_rename'].'；\r\n未勾选商品SKU数量匹配：商品编码/'.$response['goods_spec1_rename'].'/'.$response['goods_spec2_rename'].'" />',
                'field' => 'barcode_info',
                'width' => '200',
    ...
    'dataset' => 'oms/SellRecordNoticeModel::get_by_page_more',
    'queryBy' => 'searchForm',
    'idField' => 'sku_all',
    'CheckSelection' => true,
    'init'=>'nodata',
...
<script>
    ...
    $(function() {
       ...
       $('#searchForm').append($('#sku_num'));
        tableStore.on('beforeload', function(e) {
           e.params.sku_all_num = $("#sku_all_num").attr('checked')?1:0;
    ...
    function create_wave(type) {
        ...
        if(type == 1){
            var url = '?app_act=oms/sell_record_notice/create_wave_more&app_fmt=json&order_num=' + order_num; //暂时不是框架级别
        }else{
            var url = '?app_act=oms/sell_record_notice/create_wave_more_combine&app_fmt=json&order_num=' + order_num; //暂时不是框架级别
        }
        ...
        var sku_all_num = (typeof select_data[0]['sku_all_num'] != "undefined")?1:0;
        var i = 0;
        var sku_data = {};
        for (var k in select_data) {
           var key = search_key(select_data[i],sku_all_num);
            sku_data[key] = select_data[i]['express_code_all'];
            i++;
        }
        var params = {'sku_data': sku_data,'sku_all_num':sku_all_num,is_check: 2};
        create_msg();
    ...
    $.fn.stringify = function() {
        ...
        $(".btn-opt-edit_express_code").click(function () {
            get_checked(false, $(this), function (ids,idss) {
                var a =$(ids).stringify().replace(/\"/g,"'");
                var b =$(idss).stringify().replace(/\"/g,"'");
                var url = '?app_act=oms/sell_record_notice/edit_express_code&sku_list=' + a + '&sku_num='+ b + '&store_code=' + $("#store_code").val() + "&type=multi";
                params = tableStore.get('params');
                for (var key in params) {
                    if (key != 'ctl_conf' && key != 'ctl_params' && key != 'ctl_dataset' && key != 'sku_list' && key != 'sku_num') {
                        url += "&" + key + "=" + params[key];
                    }
                }
    ...
    function get_checked(isConfirm, obj, func) {
        ...
        for (var i in selecteds) {
            var arr = selecteds[i].sku_all.split(',');
            var arr_1 = selecteds[i].sku_all_num.split(',');
            ids.push(arr);
            idss.push(arr_1);
        }
                    ...
                    buttons: [
                        {
                            ...
                            elCls: 'button button-primary',
                            handler: function () {
                             //   var params = {sku_data: sku_data,sku_all_num:sku_all_num,is_check: 0};
                                var _self = this;
                              params.is_check = 0;
                              create_msg();
                              create_wave_post(url, params);
...
/*
    ...
    function search_key(row,sku_all_num){
        var key = row['sku_all'];
     	if(sku_all_num==1){
...
function remove_one(name){
    BUI.Message.Confirm('确认删除?',function(){
        param="&wave_strategy_name="+name+"&type=1";
