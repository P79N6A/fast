...
<div class="panel_div_range" style="margin-top:20px;">
<?php foreach ($response['range'] as $range_row) {?>
	...
	<input type="radio" 
            <?php if($response['strategy']['is_check']==1){?>disabled <?php }?> 
                <?php if($range_row['give_way'] == 0){ ?> checked <?php }?> 
                onchange="change_way(<?php echo $range_row['id'];?>)" 
                name="give_way_<?php echo $range_row['id'];?>" value="0"/>固定赠品
        <?php if($response['data']['type'] != 2){?>
        <input type="radio" 
            <?php if($response['strategy']['is_check']==1){?>disabled <?php }?> 
            name="give_way_<?php echo $range_row['id'];?>" value="1" 
            onchange="change_way(<?php echo $range_row['id'];?>)" 
                <?php if($range_row['give_way'] == 1){ ?> checked <?php }?>  />随机赠品
        <?php }?>
...
function change_way(range_id){
	...
	var data = {'op_gift_strategy_detail_id':<?php echo $request['_id'];?>,'range_id':range_id,'give_way':give_way};
	if (give_way == 1){
		//随机
