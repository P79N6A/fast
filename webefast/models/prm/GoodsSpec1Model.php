...
class GoodsSpec1Model extends TbModel {
	function get_table() {
		return 'goods_spec1';
	}
	...
	function get_by_id($id) {
		...
		return  $this->get_row(array('goods_spec1_id'=>$id));
	}
	function get_by_code($code) {
//		$ret = $this->get_all(array('goods_code' => $code));
//		filter_fk_name($ret['data'], array( 'spec1_code|spec1_code','spec2_code|spec2_code'));
                $sql = "select DISTINCT (spec1_code),goods_code,spec1_name from goods_sku where goods_code=:goods_code";
	...
	function save($goods_code,$spec1_code) {
		...
		$sql = "select goods_spec1_id,goods_code,spec1_code FROM {$this->table} where goods_code ='{$goods_code}' ";
		...
		$sql = 'INSERT  INTO ' . $this -> table . '(spec1_code,goods_code) VALUES' . $sql_mx ;
		$ret = $this -> db -> query($sql);
		if ($ret) {
