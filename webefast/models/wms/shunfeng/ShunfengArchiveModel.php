...
class ShunfengArchiveModel extends WmsBaseModel {
    ...
    function sync_barocde($sync_success = 1) {
        ...
        while ($page < $page_count) {
            $params = array();
            $params['CompanyCode'] =  $this->wms_cfg['company'];
            ...
            foreach ($data as $barcode_info) {
                ...
                //break;
                $params['Items'][]['Item'] = $up_data;
            }
            $this->insert_multi_duplicate('wms_archive', $log_data, " tbl_changed = VALUES(tbl_changed),is_success = VALUES(is_success) ");
            $ret = $this->biz_req('ITEM_SERVICE', $params);

            $barcode_arr = array();
            if ($ret['status'] == 2 || $ret['status'] == 1) {
                $count = count($ret['data']['Items']['Item']);
                $result = $ret['data']['Items']['Item'];
                if($count == 1 && $ret['data']['Items']['Item']['Result'] == 1){
                    $barcode_arr[] = $ret['data']['Items']['Item']['SkuNo'];
                }
                if($count > 1){
                    foreach($result as $barcode){
                        if($barcode['Result'] == 1){
                            $barcode_arr[] = $barcode['SkuNo'];
                        }
                    }
                }
    ...
    function get_barocde($page = 1, $limit = 10, $sync_success = 1) {
        ...
        if ($sync_success == 1) {
            ...
            $sql.=" WHERE 1 AND b.lastchanged>'{$tbl_changed}' AND (b.barcode is NOT null and  b.barcode<>'') ";
        } else {
            $sql = "select b.goods_code,b.spec1_code,b.spec2_code,b.sku,b.barcode,b.lastchanged,b.gb_code from goods_sku b
                LEFT JOIN   wms_archive wa ON wa.code = b.barcode  
                WHERE 1 AND wa.efast_store_code =:efast_store_code and wa.type =:type AND   is_success=0  AND (b.barcode is NOT null and  b.barcode<>'') ";
    ...
    function set_upload_info($barocde_info, &$up_data, &$log_data) {
        ...
        $up_data['SkuNo'] = $barocde_info['barcode']; //strtoupper 3.0转换成大写
        $up_data['ItemName'] = !empty($goods_info['goods_name'])?$goods_info['goods_name']:'';
        $up_data['Description'] = !empty($goods_info['brand_name'])?$goods_info['brand_name']:'';

        $up_data['ItemColor'] = !empty($spec1_info['spec1_name']) ? $spec1_info['spec1_name'] :'';
        $up_data['ItemSize'] = !empty($spec2_info['spec2_name']) ? $spec2_info['spec2_name'] :'';


        $up_data['ItemCategory']['ItemCategory1'] = empty($goods_info['category_name']) ? '' : $goods_info['category_name'];
        ...
        if (!empty($child_data)) { //子条码
            ...
            foreach ($child_data as $val) {
                if ($i < 5) {
                    $up_data['BarCode']['BarCode' . $i] = strtoupper($val['barcode']);
                } else {
                    break;
                }
                $i++;
