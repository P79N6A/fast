...
class IwmsSellRecordModel extends WmsSellRecordModel {
    ...
    function convert_data($record_code) {
        ...
        //获取订单标签
        $sql = "SELECT b.* FROM oms_sell_record_tag AS oms INNER JOIN base_order_label AS b ON b.order_label_code=oms.tag_v WHERE oms.tag_desc!='problem' AND sell_record_code=:sell_record_code  ORDER BY order_label_id DESC";
        $sql_value = array(":sell_record_code" => $record_code);
        $record_tag = $this->db->get_all($sql, $sql_value);
        $order_label_code = '';
        $order_label_name = '';
        if (!empty($record_tag)) {
            foreach ($record_tag as $tag) {
                $label_code_arr[] = $tag['order_label_code']; //标签代码
                $label_name_arr[] = $tag['order_label_name']; //标签名称
            }
            $order_label_code = implode(',', $label_code_arr);
            $order_label_name = implode(',', $label_name_arr);
        }
        ...
        $wms_order['OrderTabCode'] = $order_label_code;
        $wms_order['OrderTabName'] = $order_label_name;
        ...
        foreach ($order['goods'] as $row) {
            ...
            $order_goods['Sku'] = ($this->wms_cfg['goods_upload_type'] == 1) ? $row['sku'] : $row['barcode'];
    ...
    function conv_wms_record_info($result) {
        ...
        $ret['order_weight'] = $result['Weight'];
    ...
    function get_wms_quehuo($efast_store_code, $start_time, $end_time) {
        $this->get_wms_cfg($efast_store_code);
        if ($this->wms_cfg['api_product'] !== 'iwms') {
            return $this->format_ret(-1, '', "仓库{$efast_store_code}未对接iwms");
        }
