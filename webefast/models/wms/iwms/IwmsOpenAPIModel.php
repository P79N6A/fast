...
require_model('wms/iwms/IwmsAPIModel');

class IwmsOpenAPIModel extends IwmsAPIModel {
    ...
    INNER JOIN sys_api_shop_store s ON c.wms_config_id=s.p_id
    where s.outside_code='{$wms_store_code}' AND s.p_type=1 AND s.outside_type=1  AND c.wms_sys_code='iwms'   ";
    ...
    function ewms_orderreturnstatus_get(&$data) {
        ...
        $data = &$data['request'];
        $ret_data = load_model("wms/iwms/IwmsSellReturnModel")->conv_wms_record_info($data);
    ...
    function ewms_orderstatus_get(&$data) {
        $data = &$data['request'];
        $ret_data = load_model("wms/iwms/IwmsSellRecordModel")->conv_wms_record_info($data);
...
//        $ret = load_model('wms/WmsRecordModel')->uploadtask_order_goods_update($record_code, $record_type, $ret_data['data']['goods']);
    ...
    function set_wms_params($accesskey) {
        ...
        $data = $this->db->get_all($sql, array(':wms_system_code' => 'iwms'));
