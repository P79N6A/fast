...
class SfwmsArchiveModel extends WmsBaseModel {
    ...
    function sync_barocde($sync_success = 1) {
        ...
        while ($page < $page_count) {
            ...
            foreach ($data as $barcode_info){
                ...
                $ret = $this->biz_req('wmsMerchantCatalogService', $up_data);
                if($ret['data']['result']==2){
               //    $barcode_arr = array_keys($log_data);
                 // $this->update_is_success($barcode_arr);
                 //  var_dump('rr:',$ret);
                   //var_dump($page_count,$page,$up_data);die;
                }else{
                    $barcode_arr = array_keys($log_data);
                    $this->update_is_success($barcode_arr); 
                }
    ...
    function set_upload_info($barocde_info,&$up_data,&$log_data) {
        ...
        $info = array('company' => $this->wms_cfg['company']);
        $info['item'] = strtoupper($barocde_info['barcode']);//strtoupper 3.0转换成大写
        $info['description'] = $goods_info['goods_name']; 

        
         $info['storage_template'] = $this->get_wms_storage_template();
         
         $info['quantity_um_1'] = $info['storage_template'];   
         $info['interface_action_code'] = 'SAVE';   //NEW
         
        $info['division'] = mb_substr($spec1_info['spec1_name'].' '. $spec2_info['spec2_name'],0,25,'utf-8');
        
        $info['department'] = empty($goods_info['brand_name'])?'':$goods_info['brand_name'];
        $info['item_category1'] = empty($goods_info['category_name'])?'':$goods_info['category_name'];
       ...
       if(!empty($child_data)){ //子条码
           ...
           foreach ($child_data as  $val){
               if($i<5){
                   $info['x_ref_item_'.$i] = strtoupper($val['barcode']);
               }else{
                   break;
               }
               $i++;
