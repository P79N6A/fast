...
class HwwmsShiftOutModel extends WmsShiftOutModel {
	...
	function upload($record_code){
		...
		//发货信息
		$wms_order['Name'] = $order['store_info']['contact_person'];
		$wms_order['PostCode'] = $order['store_info']['zipcode'];
		$wms_order['Phone'] = $order['store_info']['contact_tel'];
		$wms_order['Mobile'] = $order['store_info']['contact_phone'];
		$wms_order['State'] = $this->get_area_name($order['store_info']['province']);
		$wms_order['City'] = $this->get_area_name($order['store_info']['city']);
		$wms_order['District'] = $this->get_area_name($order['store_info']['district']);
		$wms_order['Address'] = $order['store_info']['address'];
		$wms_order['Invoice'] = "N";
		...
		if ($ret['status']>0){
			return $this->format_ret(1,'','移仓出库推送成功');
		}
		return $this->format_ret(-1,$ret['data']['Response'],'移仓出库推送失败');//   return  xiugai
	}
	...
	function cancel($record_code,$efast_store_code){
        ...
        $ret = $this->biz_req($method, $wms_order);
		if ($ret['status']>0){
        	return $this->format_ret(1,'','移仓出库取消成功');
        }
        return $this->format_ret(-1,$ret['data']['Response'],'移仓出库取消失败');
	}
	...
	private function is_canceled($record_code){
		...
		$new_record_code = ctx()->db->getOne($sql, array(':record_code' => $record_code, ':record_type' => 'shift_out'));
