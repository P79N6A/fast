...
class HwwmsWbmNoticeModel extends WmsWbmNoticeModel {
	...
	function upload($record_code){
		...
		$wms_order['ShipmentType'] = 'B2B';
		//发货信息

		$wms_order['Name'] = $order['distributor']['contact_person'];
		//$wms_order['PostCode'] = $order['supplier']['zipcode'];
		$wms_order['Phone'] = $order['distributor']['phone'];
		$wms_order['Mobile'] = $order['distributor']['mobile'];
		$wms_order['Address'] = $order['distributor']['address'];
		$wms_order['Invoice'] = "N";
		$wms_order['Remark'] = $order['remark'];
		
		$sql = "select sku,lof_no,production_date,num from b2b_lof_datail where order_code = :order_code and order_type = 'wbm_notice'";
        $order_lof_goods = $this->db->get_all($sql, array(":order_code" => $order['record_code']));
		//商品详细信息
		$TotalQty = 0;
		foreach ($order_lof_goods as $row) {
			$key_arr = array('goods_name','trade_price','barcode');
			...
			$items['Sprice'] = $sku_info['trade_price'];
		...
		if ($ret['status']>0){
			return $this->format_ret(1,'','批发通知单推送成功');
		}
		return $this->format_ret(-1,$ret['data']['Response'],'批发通知单推送失败');//   return  xiugai
	}
	...
	function cancel($record_code,$efast_store_code){
        ...
        $ret = $this->biz_req($method, $wms_order);
		if ($ret['status']>0){
        	return $this->format_ret(1,'','批发通知单取消推送成功');
        }
        return $this->format_ret(-1,$ret['data']['Response'],'批发通知单取消推送失败');
	}
