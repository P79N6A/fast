...
class HwwmsWbmReturnNoticeModel extends WmsWbmReturnNoticeModel {
	...
	function upload($record_code) {
        ...
        $wms_order['ReceiptType'] = "B2B";
        $wms_order['Remark'] = $order['remark'];
        $method = 'ReceiptRequest';
        $ret = $this->biz_req($method, $wms_order);
        if ($ret['status']>0){
        	return $this->format_ret(1,'','批发退货通知推送成功');
        }
        return $this->format_ret(-1,$ret['data']['Response'],'批发退货通知推送失败');
    }
    ...
    function cancel($record_code) {
        ...
        $ret = $this->biz_req($method, $wms_order);
       	if ($ret['status']>0){
        	return $this->format_ret(1,'','批发退货通知取消推送成功');
        }
        return $this->format_ret(-1,$ret['data']['Response'],'批发退货通知取消推送失败');
    }
