...
class MidSellRecordModel extends MidAbs {
    /*
     * 获取主单信息
     */
    ...
    function get_order_info($record_code) {
        ...
        $sql = "select * from oms_sell_record where sell_record_code=:record_code ";
        $sql_values = array(
        ...
        $record_decrypt_info = load_model('sys/security/OmsSecurityOptModel')->get_sell_record_decrypt_info($data['sell_record_code']);
    ...
    function get_order_detail($record_code) {
        $sql = "select * from oms_sell_record_detail where sell_record_code=:record_code ";
        $sql_values = array(
            ':record_code' => $record_code,
        );
    ...
    function get_mid_data($record_code, $base_info = array()) {
        ...
        $mid_data = array(
            'record_code' => $ret['data']['sell_record_code'],
            'express_code' => $ret['data']['express_code'],
            'express_no' => $ret['data']['express_no'],
...
//  `mid_code` varchar(50) NOT NULL,
        ...
        $data = array_merge($base_info, $mid_data);
        return $this->format_ret(1, $data);
    }
    ...
    function order_shipping(&$order_info) {
        ...
        $shipping_status = ctx()->db->getOne($sql, array(':sell_record_code' => $order_info['record_code']));
        if ($shipping_status >= 4) {
            return $this->format_ret(1);
        }
        if(!empty($order_info['express_no'])){
            $express_code = strtoupper($order_info['express_code']);
            $sql = "select count(*) from base_express where express_code = :express_code";
            $c = CTX()->db->get_value($sql, array(':express_code' => $express_code));
            if ($c == 0) {
                return $this->format_ret(-1, '', $express_code . '配送方式代码在系统中不存在');
            }
        }

        $sell_record_code = $order_info['record_code'];
        ...
        $ret_check = $this->check_detail($order_info);
        if ($ret_check['status'] < 1) {
            return $ret_check;
        }

        $ret = load_model("oms/SellRecordOptModel")->sell_record_send($record, $detail, $sys_user, '第三方发货', 0);
        return $ret;
    }
