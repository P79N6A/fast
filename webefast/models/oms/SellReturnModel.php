...
class SellReturnModel extends TbModel {
    ...
    function get_after_service_list($filter) {
        ...
        if (isset($filter['return_label_name']) && $filter['return_label_name'] !== '') {
            ...
            $sql_join .= $tag_join . " oms_sell_return_tag AS rt ON r1.sell_return_code = rt.sell_return_code AND rt.tag_type='return_tag' ";
        }
        ...
        if (isset($filter['list_type']) && $filter['list_type'] == 'fx_return_list') {
            $sql_main .= " AND (is_fenxiao = 1 OR is_fenxiao = 2) ";
        ...
        if(isset($filter['is_fenxiao']) && $filter['is_fenxiao'] != '') {
            if($filter['is_fenxiao'] == 1) { //分销订单
                $sql_main .= " AND r1.is_fenxiao IN (1,2) ";
            } else {
                $sql_main .= " AND r1.is_fenxiao = :is_fenxiao ";
                $sql_values[':is_fenxiao'] = 0;
            }
        }
        if(isset($filter['money_arr']) && $filter['money_arr'] != '') {
            if($filter['money_arr'] == 'fx_sum_money') {
                if (isset($filter['min_money']) && $filter['min_money'] != '') {
                    $sql_main .= ' AND (fx_express_money + fx_payable_money) >= :min_money ';
                    $sql_values[':min_money'] = $filter['min_money'];
                }
                if(isset($filter['max_money']) && $filter['max_money'] != '') {
                    $sql_main .= ' AND (fx_express_money + fx_payable_money) <= :max_money ';
                    $sql_values[':max_money'] = $filter['max_money'];
                }
            } else {
                if (isset($filter['min_money']) && $filter['min_money'] != '') {
                    $sql_main .= ' AND ' . $filter['money_arr'] . ' >= :min_money ';
                    $sql_values[':min_money'] = $filter['min_money'];
                }
                if(isset($filter['max_money']) && $filter['max_money'] != '') {
                    $sql_main .= ' AND ' . $filter['money_arr'] . ' <= :max_money ';
                    $sql_values[':max_money'] = $filter['max_money'];
                }
            }
        }
        ...
        //退单是否有物流单号
        if (isset($filter['is_return_express_no']) && $filter['is_return_express_no'] != '') {
            if ($filter['is_return_express_no'] == 1) {
                $sql_main .= " AND r1.return_express_no <>''";
            } else {
                $sql_main .= " AND (r1.return_express_no = '' or r1.return_express_no is null )";
            }
        }
        ...
        $str = $this->arr_to_in_sql_value($arr, 'sell_return_code_', $sql_values);
            $sql_main .= " AND r1.sell_return_code in (" . $str . ") ";
        }
        //原单号
        if (isset($filter['sell_record_code']) && !empty($filter['sell_record_code'])) {
           $arr = $this->trim_rep_by_str($filter['sell_record_code']);
        ...
        if (isset($filter['deal_code']) && !empty($filter['deal_code'])) {
            $deal_code_list_arr = $this->trim_rep_by_str($filter['deal_code']);
            $sql_d_values = array();
            ...
            if (!empty($sell_record_data)) {
                $sell_record_arr = array_unique(array_column($sell_record_data, 'sell_record_code'));
                $sell_record_code_where = $this->arr_to_in_sql_value($sell_record_arr, 'sell_record_code', $sql_values);
                $sql_main .= " AND r1.sell_record_code in ({$sell_record_code_where}) ";
            }else{
                $sql_main .= " AND 1=2 ";
            }
        }
...
//            $sub_values[':barcode'] = "%" . $filter['barcode'] . "%";
        ...
        if (isset($filter['after_service_list_tab']) && $filter['after_service_list_tab'] !== '') {
            ...
            if ($filter['after_service_list_tab'] == 'tabs_wait_finish') {
                $sql_main .= " and r1.return_order_status = 1 and ((r1.finance_check_status = 1 and r1.return_shipping_status = 1 and r1.return_type =3) or (r1.finance_check_status = 1 and r1.return_type =1) or (r1.return_shipping_status = 1 and r1.return_type =2)) and finsih_status = 0";
            }
        ...
        if ((!isset($filter['after_service_list_tab']) || $filter['after_service_list_tab'] == '') && $filter['list_type'] != 'fx_return_list') {
            $sql_main .= " and r1.return_order_status = 0";
        }
        //退单状态
        if (isset($filter['return_order_status']) && !empty($filter['return_order_status'])) {
            $arr = explode(',', $filter['return_order_status']);
            $str = $this->arr_to_in_sql_value($arr, 'return_order_status', $sql_values);
            $sql_main .= " AND  r1.return_order_status in ({$str})";
        }

        //退货原因
        if (isset($filter['return_reason_code']) && !empty($filter['return_reason_code'])) {
            $arr = explode(',', $filter['return_reason_code']);
            $str = $this->arr_to_in_sql_value($arr, 'return_reason_code', $sql_values);
            $sql_main .= " AND  r1.return_reason_code in ({$str})";
        }
        //退款方式
        ...
        if (isset($filter['finance_reject_reason']) && !empty($filter['finance_reject_reason'])) {
            $arr = explode(',', $filter['finance_reject_reason']);
            $str = $this->arr_to_in_sql_value($arr, 'finance_reject_reason', $sql_values);
            $sql_main .= " AND  r1.finance_reject_reason in ({$str})";
        }
        ...
        if (isset($filter['return_name']) && !empty($filter['return_name'])) {
            ...
            if (!empty($customer_address_id)) {
                ...
                $sql_main .= " AND (  r1.return_name like :return_name   OR r1.customer_address_id in ({$customer_address_id_str}) ) ";
        ...
        if (isset($filter['return_mobile']) && !empty($filter['return_mobile'])) {
            ...
            if (!empty($customer_address_id)) {
                $customer_address_id_str = implode(",", $customer_address_id);
                $sql_main .= " AND ( r1.return_mobile like :return_mobile  OR r1.customer_address_id in ({$customer_address_id_str}) ) ";
        ...
        if (isset($filter['express_code']) && !empty($filter['express_code'])) {
            ...
            $str = $this->arr_to_in_sql_value($arr, 'express_code', $old_values);
            $old_sql .= " AND ro.express_code in ({$str}) ";
        }
        //原单物流单号
        ...
        if($is_distinct === TRUE){
            ...
            $group_by = " GROUP BY r1.sell_return_code ";
            $sql_main.= $group_by;
        }
        $group_by_status = empty($group_by) ? FALSE : TRUE;
        $sql_main .= " ORDER BY sell_return_id DESC ";
        $filter['page_size'] = isset($filter['pageSize']) && $filter['pageSize'] && $filter['pageSize'] != $filter['page_size'] ? $filter['pageSize'] : $filter['page_size'];
        ...
        if (isset($filter['ctl_type']) && $filter['ctl_type'] == 'export') {
            ...
            foreach ($temp_data as $key => $val) {
                ...
                $value['return_reason_name'] = oms_tb_val("base_return_reason",'return_reason_name', array('return_reason_code' => $value['return_reason_code']));
                $val['buyer_express_money'] = $val['seller_express_money'];
                if($val['is_fenxiao'] == 2) {
                    $val['refund_total_fee'] = $val['fx_express_money'] + $val['fx_payable_money'];

                    $val['adjust_money'] = 0;
                    $val['compensate_money'] = 0;
                }
                if (!empty($return_details)) {
                    foreach ($return_details as $k => &$v) {
                        ...
                        $v['data_type'] = 0;
                        $v['num'] = '';
                        $v['return_avg_money'] = $val['is_fenxiao'] == 2 ? $v['fx_amount'] : $v['avg_money'];
                        $v['change_avg_money'] = '';
                        unset($v['deal_code']);
                        unset($v['avg_money']);
        ...
        if (isset($filter['ctl_type']) && $filter['ctl_type'] == 'export' && !empty($filter['__t_user_code'])) {
            ...
            if ($is_security_role === true) {
                ...
                $log = array('user_id' =>0, 'user_code' => $filter['__t_user_code'], 'ip' => '', 'add_time' => date('Y-m-d H:i:s'), 'module' => '网络订单', 'yw_code' => '', 'operate_type' => '导出', 'operate_xq' => '售后服务单查询导出解密数据');
    ...
    function return_order_status_exp($data, $tag = ' ') {
        ...
        if ($data['return_type'] == 1 or $data['return_type'] == 3) {
    ...
    public function component($sell_return_code, $types) {
        ...
        $response = array();
        $response['is_fenxiao'] = $info['record']['is_fenxiao'];
        ...
        if (in_array("baseinfo", $types)) {
            ...
            if($data['is_fenxiao'] == 1) { //淘分销退单
                $data['refund_id_txt'] = $data['refund_id'];
            } else if($data['is_fenxiao'] == 2) { //淘分销生成的普通分销退单不跳转
                $sql = "SELECT count(*) FROM api_refund WHERE tid = :tid";
                $refund_num = $this->db->get_value($sql, array(':tid' => $data['deal_code']));
                if($refund_num > 0) {
                    $data['refund_id_txt'] = "<a href=\"javascript:openPage('平台退单','?app_act=api/sys/order_refund/view&refund_id={$data['refund_id']}','平台退单')\">{$data['refund_id']}</a>";
                } else {
                    $data['refund_id_txt'] = $data['refund_id'];
                }
            } else {
        ...
        if (in_array("return_goods", $types)) {
            ...
            foreach ($info['detail_list'] as $key => &$value) {
                ...
                foreach ($info['relation_detail_list'] as $k => $v) {
                    if ($value['sku'] == $v['sku'] && $value['deal_code'] == $v['deal_code']) {
...
//        }
    ...
    public function delete_detail($where) {
        $return_record = load_model('oms/SellReturnModel')->get_return_by_return_code($where['sell_return_code']);
        ...
        if ($data) {
            ...
            $this->add_action($return_record, '删除退单明细', '(sku:' . $row['sku'] . ')');
    ...
    public function delete_change_detail($where) {
        ...
        if (empty($ret)) {
         ...
         load_model("oms/SellReturnOptModel")->reflush_return_info($return_record['sell_return_code']);
    ...
    function add_return_goods($request) {
        ...
        if ($is_allowed_exceed['is_allowed_exceed'] == 1) {
            $ret = $this->record_check_num($request);
            if ($ret['status'] < 0) {
                return $ret;
            }
        }
        ...
        foreach ($sku_arr as $sku => $sl) {
            ...
            //售后服务单 改款 不改变金额信息
            if (isset($request['add_type']) && 'gaikuan' == $request['add_type'] && !empty($request['detail_old'] )){
                $detail_old = $request['detail_old'];
                $ins_arr1['goods_price'] = (float) @$detail_old['goods_price'];
                $ins_arr1['avg_money'] = (float) @$detail_old['avg_money'];
                $ins_arr1['fx_amount'] = (float) @$detail_old['fx_amount'];
                $ins_arr1['trade_price'] = (float) @$detail_old['trade_price'];
            }
            $ins_arr[] = $ins_arr1;
        }
    ...
    function record_check_num($data) {
        //查询订单号
        $result = $this->get_return_by_return_code($data['sell_return_code']);
        $message = '';
        if (!empty($result['sell_record_code'])) {
            //查询订单可退数量
            $return_returnable_num = load_model('oms/SellReturnOptModel')->check_returnable_num($data['sell_return_code']);
            foreach ($data['data'] as $val) {
                $ks = $result['deal_code'] . "," . $val['sku'];
                if (array_key_exists($ks, $return_returnable_num) && $return_returnable_num[$ks]['returnable_num'] < $val['num']) {
                    $key = array('barcode');
                    $sku = load_model('goods/SkuCModel')->get_sku_info($val['sku'], $key);
                    $message = '条码为：' . $sku['barcode'] . '的数量超过原订单数量。';
                    break;
                }
            }
        }
    ...
    function add_change_goods($request) {
        ...
        foreach ($sku_arr as $sku => $sl) {
            ...
            //获取换货商品sku级的价格
            $goods_price = load_model('goods/SkuCModel')->get_sku_info($sku,array('sell_price'));
            $ins_arr[] = array(
                'sell_return_code' => $sell_return_code,
                'sell_record_code' => $return_record['sell_record_code'],
                'deal_code' => $deal_code,
                'goods_code' => $find_row['goods_code'],
                'sku' => $find_row['sku'],
                'goods_price' => (float) $goods_price['sell_price'],
                'num' => $sl,
                'avg_money' => $avg_money,
                'fx_amount' => $fx_amount,
            );
        ...
        try {
            ...
            $ret = load_model('oms/SellReturnOptModel')->reflush_return_info($sell_return_code);
    ...
    function lock_oms_change_sku($return_record) {
        ...
        $lock_sku_detail = array();
        foreach ($sku_detail as $key => &$val) {
            if (isset($lof_sku_data[$val['sku']])) {
                $val['num'] -= $lof_sku_data[$val['sku']]['num'];//当前换货单的数量减去之前锁定的库存得到需要追加锁定的数量
                if ($val['num'] == 0) {
                    continue;
                }
            }
        ...
        //无锁定库存
        if (empty($lock_sku_detail)) {
            return $this->format_ret(1);
        }

        $invobj = new InvOpModel($return_record['sell_return_code'], 'oms_change', $store_code, 1, $lock_sku_detail);
    ...
    public function api_order_return_list($param) {
        ...
        $key_option = array(
            's' => array(
                'page', 'page_size', 'store_code', 'return_type', 'return_order_status', 'return_shipping_status', 'start_time', 'end_time', 'start_confirm_time', 'end_confirm_time', 'start_refund_time', 'end_refund_time', 'shop_code', 'sell_return_code', 'refund_id', 'deal_code', 'start_lastchanged', 'end_lastchanged','start_receive_time','end_receive_time')
        );
        $arr_option = array();
        //提取可选字段中已赋值数据
        $ret_option = valid_assign_array($param, $key_option, $arr_option);
        ...
        if (isset($arr_deal['sell_return_code']) && !empty($arr_deal['sell_return_code'])) {
            ...
            $sql = 'SELECT refund_record_code FROM api_refund WHERE refund_id=:_id';
            $refund_record_code = $this->db->get_value($sql, [':_id' => $arr_deal['refund_id']]);
            if (empty($refund_record_code)) {
                $sql_main .= ' AND sr.refund_id = :refund_id';
                $sql_values[':refund_id'] = $arr_deal['refund_id'];
            } else {
                $sql_main .= ' AND sr.sell_return_code = :sell_return_code';
                $sql_values[':sell_return_code'] = $refund_record_code;
            }
        ...
        if (count($ret['data']) > 0) {
            ...
            foreach ($order_list as $key => &$order) {
                $order['return_tag'] = empty($order['return_tag']) ? '' : $order['return_tag'];
                $order['tag_desc'] = empty($order['tag_desc']) ? '' : $order['tag_desc'];

                $order['shop_name'] = $order['shop_code_name'];
                unset($order['shop_code_name'],$order['shop_code_code']);
                deal_special_char($order, array('return_buyer_memo', 'return_remark'));
                $record_decrypt_info = load_model('sys/security/OmsSecurityOptModel')->get_sell_return_decrypt_info($order['sell_return_code']);
                $order = array_merge($order,$record_decrypt_info);
    ...
    private function create_sql_where($arr_deal, &$sql_values, &$sql_main) {
        ...
        $time_fld = array(
            ...
            'start_receive_time' => 'receive_time',
            'end_receive_time'=>'receive_time'
        );
        foreach ($arr_deal as $key => $val) {
            if ($key == 'page' || $key == 'page_size') {
                continue;
    ...
    private function create_default_time($arr_deal, &$sql_main, &$sql_values) {
        ...
        if(isset($arr_deal['start_receive_time'])){
            $flag = 0;
            $sql_main .= " AND sr.receive_time >= :start_receive_time";
            $sql_values[":start_receive_time"] = $arr_deal['start_receive_time'];
        }
        if(isset($arr_deal['end_receive_time'])){
            $flag = 0;
            $sql_main .= " AND sr.receive_time <= :end_receive_time";
            $sql_values[":end_receive_time"] = $arr_deal['end_receive_time'];
        }
...
//                    $value['spec2_name'] = oms_tb_val('base_spec2', 'spec2_name', array('spec2_code' => $value['spec2_code']));
    ...
    public function get_return_record_confirm_list($three_days_ago_time) {
        $sql = "SELECT sell_return_code FROM {$this->table} WHERE receive_time<:receive_time AND return_type=3 AND return_shipping_status=1  AND finance_check_status=0";
    ...
    public function scan_barcode($sell_return_code, $barcode, $param = array(), $return_package_code) {
        ...
        $sku_data = load_model('prm/SkuModel')->convert_scan_barcode($barcode, 1, 1);
        if (empty($sku_data)) {
            return $this->format_ret(-1, '', '扫描的条码在系统中不存在');
        }
        $sku = $sku_data['sku'];
        $barcode = $sku_data['barcode'];
        $sql_detail = "select sum(note_num) AS note_num from oms_sell_return_detail where sell_return_code = :sell_return_code and sku = :sku";
        ...
        if (empty($package_ret)) {
            ...
            if ($is_allowed_exceed['is_allowed_exceed'] != 1 || ($is_allowed_exceed['is_allowed_exceed'] == 1 && !empty($ret['note_num']))) {
                //插入操作
                ...
                return $this->format_ret(-1, '', '该商品在退单中不存在');
            }
        } else {
            $data = array();
            $message = '';
            $data['recv_num'] = (int) $package_ret['num'] + 1;
            if ($data['recv_num'] > $ret['note_num']) {
                if ($is_allowed_exceed['is_allowed_exceed'] == 1) {
                    return $this->format_ret(-1,'',$barcode.'条码，扫描数不能大于售后服务单申请退货数');
                }
                //提示信息
                $status = 9;
                $message = $barcode . '条码扫描数大于通知数';
            }
...
//            $ret = $this->update_detail($data, array('sell_return_code' => $sell_return_code, 'sku' => $sku));
            if ($ret['status'] == 1) {
                ...
                if (isset($unique_code) && $unique_code != '') {
                    ...
                    $ret = array('status' => 2, 'data' => $sku, 'message' => $message);
                    return $ret;
                }
                if ($status == 9) {
                    return $this->format_ret($status, $sku, $message);
                } else {
    ...
    function scan_barcode_no_return_code($params) {
        ...
        $sku_data = load_model('prm/SkuModel')->convert_scan_barcode($params['scan_barcode'], 1, 1);
        if (empty($sku_data)) {
            return $this->format_ret(-1, '', '扫描的条码在系统中不存在');
        }
        $sku = $sku_data['sku'];
        ...
        //查询退货包裹单是否有明细
        $sql_package_detail = "SELECT * FROM oms_return_package_detail WHERE return_package_code = :return_package_code AND sku = :sku";
        $package_ret = $this->db->get_row($sql_package_detail, array(':return_package_code' => $params['return_package_code'], ':sku' => $sku));
        $this->begin_trans();
        //没有明细就新增
        if (empty($package_ret)) {
            $num = 1;
            ...
            $num = (int) $package_ret['num'] + 1;
            if ($num > $package_ret['apply_num']) {
                //提示信息
                $status = 9;
                $message = $barcode . '条码扫描数大于通知数';
            }
    ...
    function no_return_code_insert_detail($return_package_code, $sku, $recv_num = 1) {
        ...
        try {
            ...
            if (!empty($pack_row)) {
                ...
                $sku_info['sku'] = str_replace('#', '_', $sku);
                $sku_info['apply_num'] = 0;
                
                $shelf_info = $this->db->get_all("select distinct bs.shelf_name from base_shelf bs left join goods_shelf gs on bs.shelf_code = gs.shelf_code where gs.store_code = :store_code and gs.sku = :sku AND bs.store_code = :store_code", array(':store_code' => $pack_row['store_code'], ':sku' => $sku_info['sku']));
                $shelf_name = '';
                foreach ($shelf_info as $val){
                    $shelf_name .= $val['shelf_name'] . ',';
                }
                $shelf_name = rtrim($shelf_name, ',');
                $sku_info['shelf_name'] = isset($shelf_name) ? $shelf_name : '';

                return $this->format_ret(3, $sku_info, $sku_info['barcode'] . '条码，包裹单没有该商品');
            } else {
                $this->rollback();
...
//            }
            ...
            $pack_sql = "select return_package_code,store_code from oms_return_package where sell_return_code =:sell_return_code and return_order_status != 2";
            ...
            if (!empty($pack_row)) {
                ...
                $this->commit();
                $sql = "select distinct bs.shelf_name from base_shelf bs left join goods_shelf gs on bs.shelf_code = gs.shelf_code where gs.store_code = :store_code and gs.sku = :sku AND bs.store_code = :store_code";
                $shelf_info = $this->db->get_all($sql, array(':store_code' => $pack_row['store_code'], ':sku' => $sku));
                $shelf_name = '';
                foreach ($shelf_info as $val){
                    $shelf_name .= $val['shelf_name'] . ',';
                }
                $sku_info['shelf_name'] = rtrim($shelf_name, ',');
                $sku_info['num'] = $recv_num;
                $sku_info['sku'] = str_replace("#","_",$sku);
                $sku_info['apply_num'] = 0;
                return $this->format_ret(3, $sku_info, $sku_info['barcode'] . '条码，包裹单没有该商品');
    ...
    function check_return_money($sell_return_code) {
        $sql = "select refund_id,sell_record_code,compensate_money,seller_express_money,adjust_money,return_avg_money,is_fenxiao,should_refunds from oms_sell_return where sell_return_code='{$sell_return_code}' ";
        $record = $this->db->get_row($sql);
        $sell_record_code = $record['sell_record_code'];
        //退单应退款
        $return_money = $record['should_refunds'];
        if(!empty($record['refund_id'])) {
            if($record['is_fenxiao'] == 1) {
                $sql = "SELECT sum(refund_fee) FROM api_taobao_fx_refund WHERE refund_record_code = :refund_record_code ";
                $api_refund_money = $this->db->get_value($sql, array(':refund_record_code' => $sell_return_code));
                $api_refund_money = empty($api_refund_money) ? 0 : $api_refund_money;
            } else if($record['is_fenxiao'] == 0) {
                $sql = "SELECT sum(refund_fee) FROM api_refund WHERE refund_record_code = :refund_record_code ";
                $api_refund_money = $this->db->get_value($sql, array(':refund_record_code' => $sell_return_code));
                $api_refund_money = empty($api_refund_money) ? 0 : $api_refund_money;
            } else {
                return $this->format_ret(1);     
            ...
            if($return_money != $api_refund_money) {
                $str = '退单应退款（' . $return_money . '）与接口退款金额（' . $api_refund_money .'）不一致，请核对。';
                $diff_money = bcsub($return_money, $record['adjust_money'], 3);
                $diff_money = bcsub($api_refund_money, $diff_money, 3);
                $data = array('diff_money' => $diff_money , 'adjust_money' => $record['adjust_money']);
                return $this->format_ret(-1, $data, $str);
            }
        }
        return $this->format_ret(1);          
    }   
    ...
    public function print_data_default($request) {
        ...
        $ecrypt_info = load_model('sys/security/OmsSecurityOptModel')->get_sell_return_decrypt_info($sell_return_code);
        $r['record'] = array_merge($r['record'],$ecrypt_info);
        //退货原因
        $sql = "SELECT return_reason_name FROM base_return_reason WHERE return_reason_code = :return_reason_code ";
        $r['record']['return_reason_name'] = $this->db->get_value($sql, array(':return_reason_code' => $r['record']['return_reason_code']));
    ...
    function update_abjust_money($params) {
        ...
        if((float)$record['adjust_money'] == (float)$params['adjust_money']) {
            return $this->format_ret(1,'','调整金额没有改变');
        }
        $this->begin_trans();
        ...
        $ret = load_model('oms/SellReturnOptModel')->reflush_return_info($params['sell_return_code']);
        if($ret['status'] < 0) {
            $this->rollback();
            return $this->format_ret(-1,'','修改失败');
        }
    ...
    function batch_confirm($id,$type){
        $msg = '';
        $err_num = 0;
        $right_num = 0;
        ...
        if($err_num>0){
    	    ...
    	    $file_name = $this->create_import_fail_files($msg);
//    	    $message .= "，错误信息<a target=\"_blank\" href=\"?app_act=sys/export_csv/download_csv&file_key={$file_name}&export_name=error\" >下载</a>";
        ...
        if($right_num == 0){
            return $this->format_ret(-1,'','确认退款失败'.$message);
        }else if($right_num>0 && $err_num>0){
            return $this->format_ret(1,'','部分确认退款成功'.$message);
        }else{
            return $this->format_ret(1,'','确认退款成功');
        }
    }
    function create_import_fail_files($msg) {
        $fail_top = array('退单号', '错误信息');
        $file_str = implode(",", $fail_top) . "\n";
        foreach ($msg as $key => $val) {
            $val_data = array($key, $val);
            $file_str .= implode(",", $val_data) . "\r\n";
        }
        $filename = md5("return_record_fail" . time());
    ...
    public function get_sell_return_decrypt_info(&$sell_return_data,$type='',$is_change=0){

        $sell_return_code = $sell_return_data['sell_return_code'];
        $ecrypt_info = load_model('sys/security/OmsSecurityOptModel')->get_sell_return_decrypt_info($sell_return_code,$type,$is_change);

        $sell_return_data = array_merge($sell_return_data,$ecrypt_info);

    }

    
    function refund_finish_cancel_return() {
        $candel_return = load_model('sys/SysParamsModel')->get_val_by_code('refund_finish_cancel_return');
        if($candel_return['refund_finish_cancel_return'] != 1) {
            return ;
        }
        $end_time = date('Y-m-d H:i:s');
        $strat_time = date('Y-m-d H:i:s', strtotime('-1 day'));
        //查询处理一天内交易关闭的已经生成售后服务单的平台退单
        $sql = "SELECT r1.refund_record_code FROM api_refund AS r1 INNER JOIN api_taobao_refund AS r2 ON r1.refund_id = r2.refund_id WHERE r2.`status` = 'CLOSED' AND r1.refund_record_code <> '' AND r1.is_change = 1 AND r1.status = 1 AND r1.last_update_time >= :strat_time AND r1.last_update_time <= :end_time GROUP BY r1.refund_record_code";
        $refund_arr = $this->db->get_all($sql, array(':strat_time' => $strat_time, ':end_time' => $end_time));
        foreach($refund_arr as $val) {
            //查询是否是合并转退单,并且有部分退单是未关闭状态,有就不作废
            $sql = "SELECT count(*) FROM api_refund AS r1 INNER JOIN api_taobao_refund AS r2 ON r1.refund_id = r2.refund_id WHERE r1.refund_record_code = :refund_record_code AND r2.`status` <> 'CLOSED' ";
            $is_close = $this->db->get_value($sql, array(':refund_record_code' => $val['refund_record_code']));
            if($is_close != 0) {
                continue;
            }
            $record = $this->get_return_by_return_code($val['refund_record_code']);
            if($record['return_order_status'] == 0) {
                load_model('oms/SellReturnOptModel')->opt_cancel($val['refund_record_code'], array('cancel_type' => 'refund_finish_cancel_return'));
            }
        }
    }
    private function trim_rep_by_str($str){
        $filter['deal_code'] = str_replace('，',',',$str);
        $record_list_arr = explode(',', $filter['deal_code']);
        foreach ($record_list_arr as &$v){
            $v = trim($v);
        }
        return array_unique($record_list_arr);
    }
