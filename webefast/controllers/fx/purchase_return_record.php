...
//        }
        $ret['data']['return_record_code'] = load_model('fx/PurchaseReturnRecordModel')->create_fast_bill_sn();
    ...
    function view(array & $request, array & $response, array & $app) {
        //主单据信息
        $ret = load_model('fx/PurchaseReturnRecordModel')->get_by_id($request['fx_purchaser_return_id']);
        ...
        $deliver = array(0=>'未入库',1=>'已入库',2=>'部分入库');
        $is_store_in = $deliver[$ret['data']['is_store_in']];
        $is_check = $ret['data']['is_check'] == 1 ? '已确认' : "未确认";
        $is_settlement = $ret['data']['is_settlement'] == 1 ? '已结算' : "未结算";
        $ret['data']['record_status'] = $is_check . ' ' . $is_store_in . ' ' . $is_settlement;
        
        $ret['data']['is_deliver_src'] = "<img src='{$iis_deliver}'>";
         //取得国家数据
    ...
    function do_add(array & $request, array & $response, array & $app) {
        ...
        $stock_adjus = get_array_vars($request, array('return_record_code', 'order_time', 'init_code', 'record_time', 'custom_code', 'store_code', 'remark','is_add_person'));
        $login_type = CTX()->get_session('login_type');
        $user_code = CTX()->get_session('user_code');
        $stock_adjus['is_add_person'] = $user_code;
        if($login_type == 2){
            $custom = load_model('base/CustomModel')->get_custom_by_user_code($user_code);
            $stock_adjus['custom_code'] = $custom['custom_code'];
        }
        $ret = load_model('fx/PurchaseReturnRecordModel')->insert($stock_adjus);
        if (isset($ret['data']) && $ret['data'] <> '') {
            $log = array('user_id' => CTX()->get_session('user_id'), 'user_code' => CTX()->get_session('user_code'), 'ip' => '', 'add_time' => date('Y-m-d H:i:s'), 'sure_status' => "未确认", 'finish_status' => '未入库', 'action_name' => "创建", 'module' => "fx_purchase_return", 'pid' => $ret['data']);
    ...
    function do_edit(array &$request, array &$response, array &$app) {
        $ret = load_model('fx/PurchaseReturnRecordModel')->edit_action($request['parameter'], array('fx_purchaser_return_id' => $request['parameterUrl']['fx_purchaser_return_id']));
        if ($ret['status'] == '1') {
            //日志
            $log = array('user_id' => CTX()->get_session('user_id'), 'user_code' => CTX()->get_session('user_code'), 'ip' => '', 'add_time' => date('Y-m-d H:i:s'), 'sure_status' => '未确认', 'finish_status' => '未入库', 'action_name' => '修改', 'module' => "fx_purchase_return", 'pid' => $request['parameterUrl']['fx_purchaser_return_id']);
            $ret1 = load_model('pur/PurStmLogModel')->insert($log);
        }
        exit_json_response($ret);
    }
    ...
    function do_check(array &$request, array &$response, array &$app) {
        ...
        $ret = load_model('fx/PurchaseReturnRecordModel')->update_check($arr[$request['type']], 'is_check', $request['id']);
        if ($ret['status'] == '1') {
            //日志
            ...
            $log = array('user_id' => CTX()->get_session('user_id'), 'user_code' => CTX()->get_session('user_code'), 'ip' => '', 'add_time' => date('Y-m-d H:i:s'), 'sure_status' => $sure_status, 'finish_status' => '未入库', 'action_name' => $action_name, 'module' => "fx_purchase_return", 'pid' => $request['id']/*,'action_note'=>$ret['data']*/);
            $ret1 = load_model('pur/PurStmLogModel')->insert($log);
        }
        exit_json_response($ret);
    }
    ...
    function do_add_detail(array & $request, array & $response, array & $app) {
        $ret = load_model('fx/PurchaseReturnRecordModel')->add_detail_goods($request['id'], $request['data'], $request['store_code']);
        exit_json_response($ret);
    }
    ...
    function do_edit_detail(array & $request, array & $response, array & $app) {
        $detail = array('record_code' => $request['return_record_code'], 'sku' => $request['sku'], 'num' => $request['num'], 'price' => $request['price']);
        $ret = load_model('fx/PurchaseReturnRecordDetailModel')->edit_detail_action($request['pid'], $detail);
        exit_json_response($ret);
    }

    function do_delete(array & $request, array & $response, array & $app) {
        $ret = load_model('fx/PurchaseReturnRecordModel')->delete($request['fx_purchaser_return_id']);
        exit_json_response($ret);
    }
    ...
    function do_delete_detail(array & $request, array & $response, array & $app) {
        ...
        $ret = load_model('fx/PurchaseReturnRecordDetailModel')->delete($request['return_record_detail_id']);
        //批次删除
        $ret1 = load_model('stm/GoodsInvLofRecordModel')->delete_pid($request['pid'], $request['sku'], 'fx_return');
        exit_json_response($ret);
    }

    //删除批次
    function do_delete_detail_lof(array & $request, array & $response, array & $app) {
        $ret = load_model('fx/PurchaseReturnRecordDetailModel')->delete_lof($request['id']);
        exit_json_response($ret);
    }
...
//    }
    ...
    function import_goods(array & $request, array & $response, array & $app) {
        ...
        $ret = load_model('fx/PurchaseReturnRecordModel')->imoprt_detail($request['id'], $file, $request['is_lof']);
        $response = $ret;
...
//    }
    ...
    function do_settlement(array &$request, array &$response, array &$app) {
        $ret = load_model('fx/PurchaseReturnRecordModel')->do_settlement($request['record_code_id']);
        exit_json_response($ret);
    }
    
    //取消结算
    function do_unsettlement(array &$request, array &$response, array &$app) {
        $ret = load_model('fx/PurchaseReturnRecordModel')->do_unsettlement($request['record_code_id']);
        exit_json_response($ret);
    }
