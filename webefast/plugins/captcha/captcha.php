		...
		Public Static Function init($W=75,$H=25,$B=''){
			...
			For($i=0;$i<4;$i++){
				self::$Txt .= substr($chars,mt_rand(0,strlen($chars)-1),1);
                         ...
                         CTX()->set_session('captcha_code',strtolower(self::$Txt));
			//$_SESSION['captcha_code']=  strtolower(self::$Txt);			// 记录session,全部小寫
...
//				}Else{//驗證碼
					...
					$txt=ImageColorAllocate($Im,35,35,35);
						...
						ImageTTFtext($Im,25,20,2,($H-4),$txt,ROOT_PATH.CTX()->app_name.'/plugins/captcha/verdanai.ttf',self::$Txt[0]);
						ImageTTFtext($Im,25,20,25,($H-4),$txt,ROOT_PATH.CTX()->app_name.'/plugins/captcha/verdanai.ttf',self::$Txt[1]);
						ImageTTFtext($Im,25,20,44,($H-4),$txt,ROOT_PATH.CTX()->app_name.'/plugins/captcha/verdanai.ttf',self::$Txt[2]);
                                                ImageTTFtext($Im,25,20,58,($H-4),$txt,ROOT_PATH.CTX()->app_name.'/plugins/captcha/verdanai.ttf',self::$Txt[3]);
					unSet($txt);
				//}
