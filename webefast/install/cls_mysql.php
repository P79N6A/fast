...
//    var $max_cache_time = 300; // 最大的缓存时间，以秒为单位
    ...
    {
	//error_log("\n====\n(".date('Y-m-d H:i:s').")".var_export($sql,true)."\n----\n",3,ROOT_PATH.'/data/error_mysql.log');
        //order_info日志
    ...
    {			
		/*
		$trac_arr = debug_backtrace();
		$trac_log_arr = array();
		foreach($trac_arr as $k=>$sub_trac){
			if($k == 0){
				continue;
			}
			$trac_log_arr[] = $sub_trac['file'].'='.$sub_trac['line'];
		}
		$trac_log_list = join(' | ',$trac_log_arr);
		*/
		$trac_log_list = '';
		$err_msg = '';
		$exception_msg = '';
        if ($message)
        {
            $err_msg = "<b>Efast info</b>: $message\n\n<br /><br />".$trac_log_list;
						$exception_msg = $message.' '.$trac_log_list;
        }
        else
        {
            $err_msg = "<b>MySQL server error report:".$trac_log_list;
            $err_msg .= print_r($this->error_message,true);
			$exception_msg = 'mysql error sql: '.$this->error_message[1]['sql'].' errno:'.$this->error_message[3]['errno'].' error:'.$this->error_message[2]['error'].' debug_backtrace:'.$trac_log_list;
        }
        throw new EfastException($exception_msg);
		error_log("[".date('Y-m-d H:i:s')."]\t".$exception_msg."\r\n", 3, 'log/log.txt');
    }
    ...
    function getMaxAll($sql){
     $num = 10000;
     $start = 0;
     $max = 10000 ;
     $arr = array();
     $limit_preg= "/limit[\s]+[\d+\s\,]*\d+$/i";
     preg_match($limit_preg, $sql, $arr);
      if(!empty($arr[0])){
    ...
    function autoExecute($table, $field_values, $mode = 'INSERT', $where = '', $querymode = '')
    {
        ...
        if ($mode == 'INSERT')
        {
            ...
            foreach ($field_names AS $value)
            {
                ...
                {
                    $fields[] = $value;
                    $values[] = "'" . trim(addslashes(stripslashes($field_values[$value]))) . "'";
                }
            }
        ...
        else
        {
            ...
            foreach ($field_names AS $value)
            {
                if (array_key_exists($value, $field_values) == true)
                {
                    $sets[] = $value . " = '" . trim(addslashes(stripslashes($field_values[$value]))) . "'";
                }
            }
...
class EfastException extends Exception {
