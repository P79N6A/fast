...
BUI.use(['bui/menu','bui/tab'],function(Menu,Tab) {
  ...
  function tabNav(moduleId,tabConfig,menuConfig,collapsed,homePage){
    ...
    menu.on('menuclick',function(ev){
      var item = ev.item;
      if(item){
        var closeable = item.get('closeable')=='undefined'?true:item.get('closeable');
        _self.tab.addTab({id: item.get('id'), title: item.get('text'), href: item.get('href'),closeable:closeable},true);
      }
  ...
  BUI.augment(mainPage,{
    ...
    openPage : function(pageInfo){
      ...
      if(module){
        ...
        if(menuItem){
          ...
          var closeable = (pageInfo.closeable=='undefined')?true:pageInfo.closeable;
    ...
    _initNavItems : function(){
      ...
      var count =  navItems.length,
        ...
        itemWidth = (typeof(_self.get('width_item'))!= "undefined")?_self.get('width_item').value:WIDTH_ITERM,// WIDTH_ITERM,
    ...
    _initContents : function () {
      ...
      $.each(modulesConfig,function(index,module){
        var id = module.id,
          temp =['<li class="dl-tab-item ks-hidden" id="M_'+id+'_menu"><div class="dl-second-nav"><div class="dl-second-tree" id="J_',id,'Tree"></div><div class="', CLS_LEFT_SLIB, '-con"><div class="', CLS_LEFT_SLIB, '"></div></div></div><div class="dl-inner-tab" id="J_',id,'Tab"></div></li>'].join('');
        new $(temp).appendTo(navContent);
      }); 
    },
    _updateContents : function (modulesConfig) {
       var _self = this,
       navContent = _self.get('navContent');
      $.each(modulesConfig,function(index,conf){
        var id = conf.id,
           module = _self.get('modules')[id];
           _self._upModuleConfig(id,conf);
           var vid='M_'+id+'_menu';
          $('#'+vid).remove();
          if(module){
              _self._removeModule(id);
          }    
          temp =['<li class="dl-tab-item ks-hidden" id="M_'+id+'_menu" ><div class="dl-second-nav"><div class="dl-second-tree" id="J_',id,'Tree"></div><div class="', CLS_LEFT_SLIB, '-con"><div class="', CLS_LEFT_SLIB, '"></div></div></div><div class="dl-inner-tab" id="J_',id,'Tab"></div></li>'].join('');
        new $(temp).appendTo(navContent);
    ...
    _upModuleConfig : function(id,newconf){
      var _self = this,
         result =null,
         modulesConfig = _self.get('modulesConfig');
      $.each(modulesConfig,function(index,conf){
        if(conf.id === id){
          modulesConfig[index] =newconf;
          _self.set('modulesConfig',modulesConfig);
          return true;
        }
      });
    ...
    _setModuleSelected : function(index,sender){
      ...
      if(currentModelIndex !==index){
        ...
        //菜单为空不切换菜单
        if(module.menu.get('el').html()==''){
            return ;
        }   
        navItems.removeClass(CLS_SELECTE);
        sender.addClass(CLS_SELECTE);
        navTabs.addClass(CLS_HIDDEN);
        var vid = $(navTabs[index]).attr('id');
        $('#'+vid).removeClass(CLS_HIDDEN);
    ...
    _setPageSelected:function(moduleIndex,pageId,isReload,search){
      ...
      if(module && pageId){
        ...
        if(item && item.get('id') === pageId){
          ...
          var closeable = item.get('closeable')=='undefined'?true:item.get('closeable');
          module.tab.addTab({id: item.get('id'), title: item.get('text'), href: href,closeable:closeable},!!isReload);
