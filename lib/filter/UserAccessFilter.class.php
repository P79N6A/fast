...
class UserAccessFilter implements IRequestFilter{
	...
	function handle_before(array & $request, array & $response, array & $app) {
		if ($app ['mode'] == 'func') {
			...
			if ($app ['grp'] == 'login')  return; //login class not control access
			...
			if ((time () - $settime > 300) && $app ['mode'] !== 'cli') {
				$value=self::makeId ( $user_id, $role_id, $group_id,$this->app_secret);
				$GLOBALS['context']->set_cookie( 'app_efid', $value);
			}
		}
	}	
	static function parseId($id,&$user_id,&$role_id,&$group_id,&$settime,$salt=APP_SALT){
		list($sign,$urg)=explode(':',base64_decode(urldecode($id)));
		if(empty($sign) || empty($urg)) return false;
		list($ef,$user_id,$role_id,$group_id,$settime)=explode('|',$urg);
		if($ef=='eF'&& $settime && $user_id && ! isset($role_id) && ! isset($group_id)) return false;
		return $sign==strtoupper(md5($salt.$urg.$salt));
	}
	static function makeId($user_id,$role_id,$group_id,$salt=APP_SALT){
	...
	static function makeSign($params,$key,$sign_method='md5',$salt=APP_SALT){
		$sign = $key;
		ksort ( $params );
