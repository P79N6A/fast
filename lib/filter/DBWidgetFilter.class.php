		...
		if($result===NULL){
			try{
				...
				$wgt_opt->cols('widget_id,widget_action,border,title,request,mdtime')
					->where_col('app_action',$action)->where_col('state',0,'>');
	...
	function handle_before(array & $request,array & $response,array & $app){
		...
		if($options && count($options)>0)
			foreach ($options as $opt){
				...
				if($this->request_is_json) $setting=json_decode($opt['request'],true);
				else $setting=eval("return {$opt['request']};");
...
class CacheWidgetCallFilter implements IReponseFilter{
	...
	function handle_after(array & $request,array & $response,array & $app){
		$context=$GLOBALS['context'];
		$cache_dir=ROOT_PATH . $context->app_name .DIRECTORY_SEPARATOR.'cache'.DIRECTORY_SEPARATOR.'app'.DIRECTORY_SEPARATOR;
		...
		$path .=DIRECTORY_SEPARATOR. basename($context->action).'.wgt';
