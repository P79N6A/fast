<?php
/**
 * 邮件发送，简单使用系统mail函数、或者socket连接发送邮件。
 * @param string to_name       接收人姓名
 * @param string to_mail      接收人邮件地址
 * @param string subject       邮件标题
 * @param string msg           邮件内容
 * @param array  $conf		       设置参数，smtp_charset,smtp_use_mail,smtp_from_name,smtp_from_mail,smtp_host,smtp_port,smtp_user,smtp_pass
 * @param boolean html         HTML邮件 or 普通邮件
 * @param boolean notification  true 要求回执， false 不用回执
 * @return boolean true成功，flase||string 失败，string失败原因。
 */
function send_mail($to_name, $to_mail, $subject, $msg,$conf, $html = false, $notification=false){
	...
	if ($use_mail && function_exists ( 'mail' )) { //使用mail函数发送邮件
