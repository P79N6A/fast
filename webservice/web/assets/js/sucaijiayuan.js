    ...
    , scrollHander: function () {
        ...
        if(tar<$("body").height()-10){
        	if(!$(windowobject).is(":animated")){//避免多次滚轮造成的多次滚动
				...
				//alert(_this.step);
  				if($(window).scrollTop()<=_this.step && _this.upOrDown=='up'&&$(window).scrollTop()==0){
	            	///////////////////////////
	         	}
	         	else{
	         		...
	         		$("#guider").animate({"top":tar+(_this.step-$("#guider").height())/2},800,function(){
		        		...
		        		if(index>=0){
							...
							$(".guider_link").eq(index+1).addClass("select");
		       			}
	         		});
	         	}
	        }
...
$(document).ready(function(){
	...
	//$(".guider_link").eq(1).addClass("select");
	...
	$("#guider").css("top",windowHeight/2-$("#guider").height()/2);
