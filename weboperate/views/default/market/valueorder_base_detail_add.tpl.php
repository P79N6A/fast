...
render_control(
    ...
    array(
        ...
        'links' => array(
            array(
                'url'   => 'market/valueorder_base/do_list',
                'title' => '增值服务订购'
            )
        )
    )
...
$view_fields = array(
    ...
    array(
        ...
        'data'       => ds_get_select_by_field('product_version', 2)
    ),
    array(
        'title'      => '销售经理',
        'type'       => 'select_pop',
...
if(isset($response['data']['api'])) {
    array_push(
        $view_fields,
        array('title'=>'密钥', 'type'=>'input', 'field' => 'api')
    );
}
?>
...
render_control(
    ...
    'col' => 2,
    'act_add'   => 'market/valueorder_base/valueorder_add',
    'data' => $response['data'],
    'rules' => array(
...
<script type="text/javascript">
    ...
    $('#val_serverid_select_pop, #val_serverid_select_img').click(function() {
        ...
        selectPopWindowval_serverid.dialog = new ESUI.PopSelectWindow('?app_act=common/select/valueserver&cpid='+cpid+'&cpversion='+cpversion+'&enable=1', 'selectPopWindowval_serverid.callback', {title: '增值服务', width: 900, height:500 ,ES_pFrmId:"<?php echo $request['ES_frmId'] ?>" }).show();
    });
</script>